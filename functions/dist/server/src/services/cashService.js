 ...
 * Format: KASA-XXXX
 */
async function generateCashAccountCode(companyId) {
 ...
 * Kasa kartı güncelle
 */
export async function updateCashAccount(input) {
    const db = await getAdminDb();
    if (!db)
        throw new Error('Firestore unavailable');
    ...
    const cashAccount = cashAccountDoc.data();
 ...
 * Kasa listesi getir
 */
export async function getCashAccounts(companyId) {
    const db = await getAdminDb();
    if (!db)
        throw new Error('Firestore unavailable');
    const snapshot = await db
...
export async function recordCashTransaction(input) {
    ...
    if (!db)
        throw new Error('Firestore unavailable');
    const { companyId, cashAccountId, type, transactionType, amount, exchangeRate, relatedEntityType, relatedEntityId, relatedEntityName, documentNumber, description, date, userId } = input;
    if (amount <= 0) {
 ...
 */
export async function getCashTransactions(companyId, cashAccountId, options) {
    const db = await getAdminDb();
    if (!db)
        throw new Error('Firestore unavailable');
    let query = db
 ...
 */
export async function transferBetweenCashAccounts(input) {
    const db = await getAdminDb();
    if (!db)
        throw new Error('Firestore unavailable');
    const { companyId, fromCashAccountId, toCashAccountId, amount, exchangeRate, description, date, userId } = input;
 ...
 */
export async function deleteCashAccount(cashAccountId, companyId, userId) {
    const db = await getAdminDb();
    if (!db)
        throw new Error('Firestore unavailable');
    const cashAccountRef = db.collection('cash_accounts').doc(cashAccountId);
 ...
 */
export async function getDefaultCashAccount(companyId) {
    const db = await getAdminDb();
    if (!db)
        throw new Error('Firestore unavailable');
    const snapshot = await db
