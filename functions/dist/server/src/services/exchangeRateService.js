 ...
 */
export async function getExchangeRate(currency, date, source = 'TCMB') {
    if (currency === 'TRY') {
        return 1.0;
    }
    if (source === 'TCMB') {
 ...
 * @returns Döviz kuru
 */
async function fetchTCMBRate(currency, date) {
    try {
 ...
 * @returns Döviz kuru
 */
async function fetchXERate(currency) {
    try {
 ...
 * @returns Geçerli mi?
 */
export function validateExchangeRate(exchangeRate, currency, invoiceDate, exchangeRateDate) {
    if (currency === 'TRY') {
