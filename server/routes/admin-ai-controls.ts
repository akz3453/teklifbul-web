...
async function writeSystemAuditLog(
  db: any,
  data: {
    type: 'aiControls.global' | 'aiControls.panic';
    action: string;
    reason: string | null;
    actorUid: string;
 ...
 * Teklifbul Rule v3.11 - Audit logging
 */
async function writeCompanyAuditLog(
  db: any,
  companyId: string,
  data: {
    action: string;
 ...
 */
async function getActorInfo(db: any, userId: string): Promise<{ email: string | null; name: string | null }> {
  try {
...
router.post('/set-global', validateRequest({ body: setGlobalSchema }), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const { globalAiDisabled, reason } = req.body as any;
...
router.post('/set-panic', validateRequest({ body: setPanicSchema }), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const { panicMode, reason } = req.body as any;
    const userId = req.user?.uid;
 ...
 */
router.get('/company', async (req: AuthenticatedRequest, res) => {
  try {
...
router.post('/company', validateRequest({ body: setCompanySchema }), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const { companyId, companyAiDisabled, reason } = req.body as any;
    const userId = req.user?.uid;
