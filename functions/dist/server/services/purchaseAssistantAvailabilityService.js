...
function getCacheKey(companyId, planId) {
...
function uniqKey(provider, model) {
...
export async function loadAiModelCatalog(db) {
    try {
        ...
        logger.warn('ai_model_catalog load failed, using fallback list', { error: e?.message || e });
    }
 ...
 */
export async function isModelFreeEligible(provider, model) {
    try {
        ...
        logger.warn('isModelFreeEligible check failed', { provider, model, error: e?.message || e });
        return false;
    }
}
export async function computeAvailableModels(params) {
    ...
    for (const p of purchaseDocs) {
        ...
        const unlockedBy = snap?.id && snap?.name
            ? { packageId: String(snap.id), packageName: String(snap.name) }
            : p?.packageId
                ? { packageId: String(p.packageId), packageName: String(p.packageId) }
                : undefined;
        const allowedProviders = Array.isArray(snap.allowedProviders)
            ? snap.allowedProviders.map((x) => String(x || '').trim()).filter(Boolean)
            : [];
        ...
        for (const provider of allowedProviders) {
            ...
            for (const m of models) {
                ...
                const modelTier = m.tier || undefined; // Optional tier from catalog
...
export function invalidateAvailableModelsCache(companyId) {
...
export function invalidateAvailableModelsCacheForCompany(companyId) {
    invalidateCache(companyId);
}
export async function getCompanyPlanFlags(companyId) {
    const db = await getAdminDb();
    if (!db)
        return { isPremiumPlus: false, planId: 'free' };
...
export async function ensureActiveCompanyModelValid(params) {
    ...
    const db = await getAdminDb();
    if (!db)
        return { provider, model, modelAutoDowngraded: false };
    const plan = await getCompanyPlanFlags(companyId);
