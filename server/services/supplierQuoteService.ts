...
export async function generateQuoteToken(
    ...
    try {
        ...
        const tokenDoc: Omit<QuoteToken, 'id'> = {
            token,
            requestId,
 ...
 */
export async function getRequestByToken(token: string): Promise<{
    success: boolean;
    error?: string;
...
export async function getQuotesForRequest(
    requestId: string,
    companyId: string
): Promise<{ success: boolean; quotes?: SupplierQuote[]; error?: string }> {
    try {
        const db = await getAdminDb();
        if (!db) {
            return { success: false, error: 'Veritabanı bağlantısı yok' };
        ...
        const quotes: SupplierQuote[] = snapshot.docs.map((doc: FirebaseFirestore.QueryDocumentSnapshot) => {
            const data = doc.data();
            return {
                id: doc.id,
 ...
 */
export async function getTokensForRequest(
    requestId: string,
    companyId: string
): Promise<{ success: boolean; tokens?: QuoteToken[]; error?: string }> {
    try {
        const db = await getAdminDb();
        if (!db) {
            return { success: false, error: 'Veritabanı bağlantısı yok' };
        ...
        const tokens: QuoteToken[] = snapshot.docs.map((doc: FirebaseFirestore.QueryDocumentSnapshot) => {
            const data = doc.data();
            return {
                id: doc.id,
                token: data.token,
