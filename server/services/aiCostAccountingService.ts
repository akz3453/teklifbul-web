...
import { loadAiModelCatalog, type CatalogModel } from './purchaseAssistantAvailabilityService.js';
 ...
 */
export async function getUsdTryRate(db: any): Promise<number> {
  try {
 ...
 */
export async function getUsdTryRateSnapshot(db: any, month: string): Promise<number | null> {
  try {
 ...
 */
export function getModelCostPer1kUsd(
  provider: string,
  model: string,
 ...
 */
export function estimateCostUsdFromTokens(tokens: number, costPer1kUsd: number | null): number | null {
 ...
 */
export function toTry(usd: number | null, usdTryRate: number): number | null {
  if (usd === null || usd < 0) {
    return null;
  }
...
export async function computeLedgerCostMeta(params: {
  ...
  costUsd: number | null;
  costTry: number | null;
  usdTryRateUsed: number;
  costComputedAt: string;
  costVersion: string;
}> {
 ...
 */
export function summarizeCostsFromLedgerEntries(entries: any[]): {
  paidTokens: number;
  freeTokens: number;
  costUsdKnown: number;
  costTryKnown: number;
  costUsdUnknownTokens: number;
  unknownCostEntriesCount: number;
} {
  let paidTokens = 0;
  let freeTokens = 0;
  let costUsdKnown = 0;
 ...
 */
export async function checkFxDriftWarning(
  db: any,
  month: string
): Promise<{
  month: string;
  current: number;
  snapshotOrNull: number | null;
  pctDiff: number;
} | null> {
  try {
