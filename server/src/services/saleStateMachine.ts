 ...
 */
export const VALID_TRANSITIONS: Record<SaleStatus, SaleStatus[]> = {
...
export function assertValidStatusTransition(
  ...
  if (!validateStatusTransition(currentStatus, newStatus)) {
    throw new Error(
      `Invalid status transition: ${currentStatus} → ${newStatus}. Allowed transitions: ${VALID_TRANSITIONS[currentStatus]?.join(', ') || 'none'}`
    );
  }
  return true;
}
 ...
 */
export function requiresApproval(status: SaleStatus): boolean {
