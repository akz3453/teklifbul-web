 ...
 */
import { previewXlsx } from "./importParser.js";
import { previewDocx } from "./docxParser.js";
import { previewPdf } from "./pdfParser.js";
import { enrichCandidate, normalizeKey, parseDateTR, parseNumberTR } from "./normalization.js";
import { getSupplierMemoryStore } from "./supplierMemory.js";
import { scoreCandidate } from "./scorers.js";
// Teklifbul Rule v1.0 - Template detection
import { detectTemplate } from "./templateDetector.js";
...
async function parseRawDocument(buffer, options) {
...
function gatherColumnLabels(doc) {
...
function pickSampleValue(doc, column) {
    if (doc.matrix && doc.matrix.length) {
...
function assignColumns(doc, supplierAliases, minConfidence) {
    ...
    const unique = [];
    const finalUsedColumns = new Set();
 ...
 */
function createColumnMappings(doc, assignments) {
    const labels = gatherColumnLabels(doc);
...
function mapDemandFields(doc, supplierAliases, minConfidence) {
    const fieldResults = {};
    ...
    for (const target of DEMAND_FIELDS) {
        ...
        candidates.forEach((cand) => {
            ...
            if (!best || total > best.score)
                best = { score: total, candidate: cand };
        });
        const finalBest = best;
        if (finalBest) {
            const { candidate, score } = finalBest;
            let value = candidate.value;
            if (target === "demandDate" || target === "dueDate")
                value = parseDateTR(value) || value;
            if (target === "currency")
                value = (candidate.value || doc.demand?.currency || "TRY").toString().toUpperCase();
            if (target === "note")
...
function mapItems(doc, assignments, minConfidence) {
    if (!doc.matrix || !doc.matrix.length) {
...
export async function mapDocument(buffer, options = {}) {
    ...
    assignments.forEach((assign) => {
        ...
        if (label)
            supplierStore.remember(options.supplierId || null, label, assign.field, assign.score);
    });
    Object.entries(demandFields).forEach(([field, result]) => {
        if (result.sourceLabel)
            supplierStore.remember(options.supplierId || null, result.sourceLabel, field, result.confidence);
    });
    // Teklifbul Rule v1.0 - Talep bilgileri için de eşleştirme oluştur
    // Duplicate kontrolü: Aynı columnLabel'ı tekrar ekleme
    const seenLabels = new Set();
    const demandMappings = Object.entries(demandFields)
        .map(([field, result]) => ({
        field: field,
        ...
        if (!m.columnLabel || m.columnLabel === m.field)
            return false;
        if (seenLabels.has(m.columnLabel))
            return false; // Duplicate kontrolü
