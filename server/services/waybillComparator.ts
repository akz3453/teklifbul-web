...
import type { ParsedWaybill, WaybillLine } from './waybillParser.js';

export interface LineMatch {
  supplierLine: WaybillLine;
  buyerLine?: WaybillLine;
  nameSimilarity: number;
  qtyDelta: number | null;
}

export interface WaybillComparisonResult {
  score: number;
...
function matchLines(supplierLines: WaybillLine[], buyerLines: WaybillLine[]): LineMatch[] {
  const matches: LineMatch[] = [];
  for (const sLine of supplierLines) {
    let best: WaybillLine | undefined;
    let bestScore = 0;
...
export async function compareWaybills(supplier: ParsedWaybill, buyer: ParsedWaybill): Promise<WaybillComparisonResult> {
  ...
  const score =
    (numberSimilarity * SIMILARITY_CONFIG.numberWeight) +
  ...
  const status: WaybillComparisonResult['status'] =
    autoApprove ? 'match' : (score >= 0.6 ? 'warning' : 'mismatch');
