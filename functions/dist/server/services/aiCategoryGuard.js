...
import { buildMaterialProfileFromItems, evaluateCategoryCompatibility, suggestCategoriesForProfile } from '../../category-rules.js';
const SYSTEM_PROMPT = `
...
export async function validateCategorySelection(payload) {
    ...
    try {
        ...
        const incompatible = compatibility.filter(entry => entry.result && (entry.result.status === 'blocked' || entry.result.status === 'mismatch'));
        ...
        const result = await sendGroqChat([{ role: 'user', content: userPrompt }], {
