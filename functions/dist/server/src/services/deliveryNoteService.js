...
export async function createDirectDeliveryNoteDraft(options) {
    ...
    const { companyId, userId, customerId, items, shipDate, shipToAddress, fromLocationId, fromLocationName, requestId } = options;
    if (!customerId) {
 ...
 */
export async function updateDeliveryNote(deliveryNoteId, userId, companyId, updateData) {
    const db = await getAdminDb();
    if (!db) {
        throw new Error('Firestore unavailable');
    }
 ...
 */
export async function prepareDeliveryNote(deliveryNoteId, userId, companyId) {
    const db = await getAdminDb();
    if (!db) {
        throw new Error('Firestore unavailable');
    }
 ...
 */
export async function sendDeliveryNote(deliveryNoteId, userId, companyId) {
    const db = await getAdminDb();
    if (!db) {
        throw new Error('Firestore unavailable');
    }
    ...
    return await db.runTransaction(async (transaction) => {
        ...
        // Provider'a gönder
        const sendResult = await provider.sendDespatch(deliveryNote.snapshot, deliveryNote.edoc, credentials);
 ...
 */
export async function syncDeliveryNoteStatus(deliveryNoteId, userId, companyId) {
    const db = await getAdminDb();
    if (!db) {
        throw new Error('Firestore unavailable');
    }
    ...
    return await db.runTransaction(async (transaction) => {
        ...
        const statusResult = await provider.getDespatchStatus(deliveryNote.edoc.externalId, credentials);
        // Status mapping
 ...
 */
export async function getDeliveryNotePdf(deliveryNoteId, userId, companyId) {
    const db = await getAdminDb();
    if (!db) {
        throw new Error('Firestore unavailable');
    }
    ...
    const { provider, credentials } = await getEdocProvider(companyId);
    // Provider'dan PDF al
    const pdfResult = await provider.getDespatchPdf(deliveryNote.edoc.externalId, credentials);
    // PDF URL'i kaydet
 ...
 */
export async function cancelDeliveryNote(deliveryNoteId, userId, companyId) {
    const db = await getAdminDb();
    if (!db) {
        throw new Error('Firestore unavailable');
    }
    ...
    return await db.runTransaction(async (transaction) => {
        ...
        const cancelResult = await provider.cancelDespatch(deliveryNote.edoc.externalId, credentials);
