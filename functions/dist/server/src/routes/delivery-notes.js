 ...
 */
router.post('/', requirePermission('edespatch.create'), async (req, res) => {
    try {
 ...
 */
router.post('/direct', requirePermission('edespatch.create'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.get('/:id', validate({ params: deliveryNoteIdParamsSchema }), requirePermission('edespatch.view'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
...
}), requirePermission('edespatch.create'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
...
router.post('/:id/send', validate({
    params: deliveryNoteIdParamsSchema,
    body: companyIdBodySchema
}), edocSendLimiter, requirePermission('edespatch.send'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.get('/:id/status', validate({ params: deliveryNoteIdParamsSchema }), edocStatusLimiter, requirePermission('edespatch.status'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 * Permission: edespatch.view
 */
router.get('/:id/pdf', validate({ params: deliveryNoteIdParamsSchema }), edocPdfLimiter, requirePermission('edespatch.view'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
...
router.post('/:id/cancel', validate({
    params: deliveryNoteIdParamsSchema,
    body: companyIdBodySchema
}), edocCancelLimiter, requirePermission('edespatch.cancel'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
