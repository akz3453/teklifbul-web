...
export interface CachedUserDoc {
  exists: boolean;
  data: any | null;
}
...
export async function getCachedUserDoc(
  userId: string,
  req?: Request
): Promise<CachedUserDoc> {
  if (!userId) return { exists: false, data: null };
  ...
  if (req) {
    const reqCached = getFromReqCache<CachedUserDoc>(req, reqKey);
    if (reqCached) {
  ...
  const ttlCached = ttlCache.get<CachedUserDoc>(ttlKey);
  if (ttlCached) {
    if (req) {
  ...
  try {
    ...
    if (!db) {
      ...
      const empty: CachedUserDoc = { exists: false, data: null };
      return empty;
    }

    const snap = await db.collection('users').doc(userId).get();
    const result: CachedUserDoc = {
      exists: snap.exists,
 ...
 */
export function invalidateUserDoc(userId: string): void {
  if (!userId) return;
