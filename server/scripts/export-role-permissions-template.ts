...
const PERM_KEY_BY_LABEL: Record<string, string> = {
...
function resolveGroup(label: string): string {
  ...
  if (l.startsWith('teklif')) return 'Teklifler';
  if (l.startsWith('talep') || l.startsWith('şirket içi talep') || l.startsWith('yeni satın alma talebi')) return 'Talepler';
  if (l.startsWith('stok')) return 'Stok';
  if (l.startsWith('stok hareketi')) return 'Stok';
  if (l.startsWith('hakediş')) return 'Hakediş';
  // Ödeme talep ve e-fatura / e-irsaliye izinlerini Hakediş sekmesi altında grupla
  if (l.startsWith('ödeme') || l.startsWith('odeme')) return 'Hakediş';
  if (l.startsWith('e-fatura') || l.startsWith('efatura') || l.startsWith('fatura ')) return 'E-Belgeler';
  if (l.startsWith('e-irsaliye') || l.startsWith('eirsaliye') || l.startsWith('irsaliye')) return 'E-Belgeler';
  if (l.startsWith('e-belge')) return 'E-Belgeler';
  if (l.startsWith('satış') || l.startsWith('satis')) return 'Satışlar';
  if (l.startsWith('firma') || l.startsWith('şirket profili') || l.startsWith('adres yönetimi') || l.startsWith('kullanıcı') || l.startsWith('onay limiti')) return 'Ayarlar & Kullanıcı Yönetimi';
  if (l.startsWith('premium') || l.startsWith('ai ')) return 'Premium & AI';
  // Fallback
  return 'Diğer';
}
...
function main() {
  ...
  const permHeaders: string[] = headerRow.slice(2).map((v: any) => String(v || '').trim()).filter(Boolean);
  ...
  const roles: { roleKey: string; label: string }[] = [];
  const matrixDefaults: Record<string, Record<string, boolean>> = {};
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const permValues: Record<string, boolean> = {};
    for (let colIndex = 2; colIndex < headerRow.length; colIndex++) {
      ...
      let value: boolean;
      if (cell === 'EVET') value = true;
      else if (cell === 'HAYIR') value = false;
