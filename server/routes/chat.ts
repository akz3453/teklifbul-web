  ...
  try {
    ...
    if (useCompanyFlow) {
      ...
      const effectiveProvider =
        companyProvider === 'free_local' ? 'groq' : companyProvider;
      const effectiveModel =
        companyProvider === 'free_local' ? DEFAULT_FREE_AI.model : model;
      const effectiveFree =
        companyProvider === 'free_local'
          ? true
    ...
    const hasTokens = (p: AIProvider) =>
      allPacks.some((pack) => pack.provider === p && pack.remainingTokens > 0);

    let provider: AIProvider = DEFAULT_FREE_AI.provider;
    let freeMode = true;
    let model: string = DEFAULT_FREE_AI.model;
