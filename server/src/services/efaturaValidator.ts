...
/**
 * E-Fatura Senaryo Tipleri (TR e-fatura standartları)
 */
export type EFaturaScenario =
  | 'BASIC' // Temel fatura
  | 'RETURN' // İade
  | 'EXPORT' // İhracat
  | 'IMPORT' // İthalat
 ...
 * E-Fatura Tip
 */
export type EFaturaType = 'SATIS' | 'IADE' | 'ISTISNA' | 'OZEL_MATRAH';

/**
 * E-Fatura Validasyon Sonucu
 */
...
export function validateEInvoiceData(invoiceData: {
  ...
  if (invoiceData.scenario) {
    const validScenarios: EFaturaScenario[] = [
      'BASIC',
      'RETURN',
      'EXPORT',
      'IMPORT',
  ...
  if (invoiceData.type) {
    const validTypes: EFaturaType[] = ['SATIS', 'IADE', 'ISTISNA', 'OZEL_MATRAH'];
 ...
 */
export function validateRequiredFields(invoiceData: any): string[] {
