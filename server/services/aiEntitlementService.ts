...
export type ProviderEntitlements = {
  ...
  allowedModelPrefixes: string[];
  allowedTiers: string[];
  isProviderWide: boolean; // true if no specific entitlements (legacy behavior)
};

// In-memory cache: 30s TTL
interface CacheEntry {
  data: Map<string, ProviderEntitlements>; // providerKey -> entitlements
  expiresAt: number;
}
...
export function isModelAllowed(
  entitlements: ProviderEntitlements | null,
  provider: string,
  model: string,
  modelTier?: string
): boolean {
  ...
  if (entitlements.isProviderWide) return true;
  
  // Check exact model match
  if (entitlements.allowedModels.includes(model)) return true;
  ...
  // Check tier match
  if (modelTier && entitlements.allowedTiers.includes(modelTier)) return true;
 ...
 */
function getLegacyProviderWideEnabled(): boolean {
 ...
 */
function mergeEntitlements(entitlementsList: Entitlement[], hasLegacyPackages: boolean = false): ProviderEntitlements {
  const merged: ProviderEntitlements = {
    providerKey: '',
    allowedModels: [],
...
export async function getCompanyEntitlements(
  companyId: string
): Promise<Map<string, ProviderEntitlements>> {
  // Check cache
  const now = Date.now();
  ...
  try {
    ...
    logger.warn('[Entitlements] Failed to load purchases', { companyId, error: (e as any)?.message || e });
    return new Map();
  }
  
  // Group entitlements by provider
  const entitlementsByProvider = new Map<string, Entitlement[]>();
  const legacyPackagesByProvider = new Map<string, boolean>(); // Track if provider has legacy packages
  ...
  for (const purchase of purchaseDocs) {
    ...
    const hasEntitlements = 
      (entitlement.allowedModels && entitlement.allowedModels.length > 0) ||
  ...
  const result = new Map<string, ProviderEntitlements>();
...
export async function getProviderEntitlements(
  companyId: string,
  providerKey: string
): Promise<ProviderEntitlements | null> {
 ...
 */
export function clearEntitlementCache(companyId: string): void {
