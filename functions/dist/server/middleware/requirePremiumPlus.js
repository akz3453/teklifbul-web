...
function isPremiumPlusPlanId(planId) {
 ...
 */
export async function requirePremiumPlus(req, res, next) {
    try {
            ...
            if (db && userDoc.exists) {
                ...
                if (companyId && !companyId.startsWith('solo-')) {
                    ...
                    if (companyDoc.exists) {
                        ...
                        const companyPlanId = String(companyData.planId ||
                            companyData.plan ||
                            companyData.subscriptionPlanId ||
                            companyData.subscription?.planId ||
                            'free');
                        const expiresAt = companyData.premiumExpiresAt ||
                            companyData.expiresAt ||
                            companyData.subscription?.expiresAt;
                        if (isPremiumPlusPlanId(companyPlanId)) {
                            if (expiresAt) {
                                const parsed = expiresAt.toDate ? expiresAt.toDate() : new Date(expiresAt);
        ...
        req.subscription = {
            ...
            currentPeriodEnd: companyExpiresAt || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000),
