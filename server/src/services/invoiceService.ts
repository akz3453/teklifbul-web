...
export async function createInvoiceDraftFromSale(options: {
  ...
  return await db.runTransaction(async (transaction) => {
    ...
    // Item snapshot
    const itemSnapshots: ItemSnapshot[] = sale.items.map((item: any) => ({
      sku: item.sku || '',
      name: item.name || '',
    ...
    const datesSnapshot: DatesSnapshot = {
      issueDate: now,
    ...
    const invoiceData: Omit<Invoice, 'id'> = {
      ...
      status: 'draft',
      invoiceType: invoiceTypeDefault as InvoiceType | null,
      scenario: scenarioDefault as ScenarioType | null,
    ...
    const cleanObject = (obj: any) => {
      Object.keys(obj).forEach(k => {
        ...
        else if (obj[k] && typeof obj[k] === 'object' && !obj[k].toDate) cleanObject(obj[k]);
      });
    };
...
export async function createDirectInvoiceDraft(options: {
  ...
  return await db.runTransaction(async (transaction) => {
    ...
    const itemSnapshots: ItemSnapshot[] = items.map((item, idx) => {
...
export async function updateInvoice(
  ...
  return await db.runTransaction(async (transaction) => {
    ...
    const immutableStatuses: InvoiceStatus[] = ['sent', 'accepted'];
...
export async function recordPayment(
  ...
  paymentData: {
    amount: number;
    date: Date;
    method: string; // nakit, havale, kredi_karti, cek, senet
    reference?: string;
  }
): Promise<void> {
  const db = await getAdminDb();
  ...
  if (saleId) {
    ...
    const currency = invoiceData?.currency as string | undefined;
    const invoiceNumber = invoiceData?.number as string | undefined;
...
export async function sendInvoice(
  ...
  return await db.runTransaction(async (transaction) => {
    ...
    // Provider'a gönder
    const sendResult = await provider.sendInvoice(
      invoice.snapshot,
      invoice.edoc,
      credentials
    );
...
export async function syncInvoiceStatus(
  ...
  userId: string,
  companyId: string
): Promise<{ status: InvoiceStatus; ettn?: string; pdfUrl?: string }> {
  const db = await getAdminDb();
  if (!db) {
    throw new Error('Firestore unavailable');
  }
  ...
  return await db.runTransaction(async (transaction) => {
    ...
    const statusResult = await provider.getInvoiceStatus(
      invoice.edoc.externalId,
      credentials
    );
...
export async function getInvoicePdf(
  ...
  // Provider'dan PDF al
  const pdfResult = await provider.getInvoicePdf(
    invoice.edoc.externalId,
    credentials
  );
