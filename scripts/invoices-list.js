/**
 * Invoices List - Faturalama Modülü
 * Şirketin oluşturduğu/aldığı tüm faturaların listelenmesi.
...
function renderRows() {
  ...
  tbody.innerHTML = state.rows.map((row) => {
    ...
    const total = row?.snapshot?.totals?.grandTotal ?? row?.snapshot?.totals?.totalAmount ?? 0;
    ...
    return `
      <tr data-id="${escape(row.id)}">
        ...
        <td>${escape(buyer)}</td>
        <td>${escape(formatCurrency(total))}</td>
        <td>${statusBadge(row.status)}</td>
        <td>${escape(formatDate(issued))}</td>
        <td>${escape(type)}</td>
        <td>
          <a class="btn btn-secondary" href="/pages/invoice-detail.html?id=${encodeURIComponent(row.id)}">Görüntüle</a>
        </td>
      </tr>
    `;
  }).join('');
  ...
  tbody.querySelectorAll('tr[data-id]').forEach((tr) => {
    tr.addEventListener('click', (ev) => {
      ...
      if (id) window.location.href = `/pages/invoice-detail.html?id=${encodeURIComponent(id)}`;
    });
  });
}
...
async function loadPage({ direction = 'first' } = {}) {
  ...
  logger.group('Invoices listesi yükleniyor');
  try {
    const tbody = qs('#invoicesTableBody');
    tbody.innerHTML = `<tr><td colspan="7" class="empty-state">Yükleniyor...</td></tr>`;
    ...
    if (direction === 'next' && state.pageStack.length > 0) {
      ...
      state.pageStack.pop(); // Mevcut cursor
      const cursor = state.pageStack[state.pageStack.length - 1];
    ...
    logger.info('Invoices listesi yüklendi', { count: state.rows.length });
  } catch (err) {
    logger.error('Invoices listesi yükleme hatası', err);
    if (String(err?.code) === 'failed-precondition') {
      toast.error('Listeleme için Firestore index eksik olabilir. Lütfen index oluşturulmasını bekleyin.');
    } else {
      toast.error(`Faturalar yüklenemedi: ${err?.message || err}`);
    }
    qs('#invoicesTableBody').innerHTML =
      `<tr><td colspan="7" class="empty-state">Yüklenirken hata oluştu.</td></tr>`;
  } finally {
    logger.end();
  }
}
...
(async () => {
  try {
    ...
    logger.error('Invoices list init hatası', err);
