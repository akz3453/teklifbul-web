...
async function writeAuditLog(
  ...
  companyId: string,
  data: {
    action: 'force_free_on' | 'force_free_off' | 'set_daily_cap' | 'clear_daily_cap' | 'reset_guardrails'; // Teklifbul Rule v3.5.1
    reason: string | null;
    actorUid: string;
 ...
 */
router.post('/apply', validateRequest({ body: applySchema }), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const { companyId, action, reason, dailyPaidTokenCap } = req.body as any;
