...
import type { TextResult } from './waybillText.js';
...
function normalizeValue(val?: string | null): string | undefined {
  if (!val) return undefined;
  const trimmed = val.toString().trim();
  return trimmed.length > 0 ? trimmed : undefined;
}

function parseNumber(text: string): string | undefined {
...
function parseParty(lines: string[], rx: RegExp): string | undefined {
...
function tryParseItemFromLine(line: string): WaybillLine | null {
  if (!line || line.length < MIN_LINE_LENGTH) return null;
...
export async function parseWaybill(textResult: TextResult): Promise<ParsedWaybill> {
