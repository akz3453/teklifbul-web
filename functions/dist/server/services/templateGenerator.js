 ...
 */
export async function generateDemandTemplate() {
    ...
    try {
        ...
        workbook.properties.customProperties = [
 ...
 * @returns ExcelJS Workbook buffer
 */
export async function generateCustomDemandTemplate(demandFields, itemFields) {
 ...
 */
function setupCustomInfoSheet(sheet, enabledFields) {
    // Stil yardımcı fonksiyonları
    const applyCellStyle = (cell, options) => {
        if (options.font)
            cell.font = options.font;
 ...
 */
function addDataValidationToInfoSheet(sheet, cellAddress, validationType) {
    let validation = null;
    ...
    if (validation) {
        ...
        sheet.dataValidations.add(cellAddress, validation);
    }
}
/**
 ...
 */
function addDataValidation(sheet, cellAddress, validationType) {
    let validation = null;
    switch (validationType) {
        case 'priority':
            validation = {
 ...
 */
function setupItemsSheet(sheet) {
    // Başlık
    ...
    sheet.getColumn(6).eachCell((cell, rowNumber) => {
        if (rowNumber >= 3) { // A3'ten başlıyor (A1 başlık, A2 header)
            // Type assertion: ExcelJS TypeScript tiplerinde dataValidations tanımlı değil ama runtime'da çalışıyor
            sheet.dataValidations.add(cell.address, unitValidation);
        }
    });
 ...
 * Teklifbul Rule v1.0 - Seçilen alanlara göre şablon oluşturma
 */
function setupCustomItemsSheet(sheet, enabledFields) {
    // Başlık
    sheet.getCell('A1').value = 'KALEMLER';
    sheet.getCell('A1').font = { bold: true, size: 16, color: { argb: 'FF1E3A8A' } };
    sheet.getCell('A1').alignment = { horizontal: 'center', vertical: 'middle' };
...
function setupBidSheet(sheet) {
    ...
    sheet.dataValidations.add('D3', currencyValidation);
    ...
    sheet.dataValidations.add('F3', shippingValidation);
    ...
    sheet.dataValidations.add('M3', paymentValidation);
    // C sütunundaki eski Ödeme Planı dropdown'ını kaldır (artık M3'te)
    // C8'deki dropdown'ı kaldırmak için dataValidation'ı temizle
    try {
        const oldValidation = sheet.dataValidations.get('C8');
        if (oldValidation) {
            sheet.dataValidations.remove('C8');
        }
    }
    catch (e) {
    ...
    for (let row = firstProductRow; row <= lastProductRow; row++) {
        sheet.dataValidations.add(`M${row}`, vatValidation);
    }
}
