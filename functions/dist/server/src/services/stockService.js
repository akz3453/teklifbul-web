 ...
 */
export async function createStockMovements(saleItems, companyId, saleId, userId, allowNegativeStock = false) {
 ...
 */
export async function createReversalMovements(saleItems, movementIds, companyId, saleId, userId) {
    const db = await getAdminDb();
    if (!db) {
        throw new Error('Firestore unavailable');
    }
    ...
    const originalMovements = await Promise.all(movementIds.map(async (movementId) => {
    ...
    for (let i = 0; i < saleItems.length; i++) {
        ...
        const reversalUnitCost = originalMovement.unitCost || oldAvg;
 ...
 */
export async function createPurchaseMovements(purchaseItems, companyId, invoiceId, userId) {
    const db = await getAdminDb();
    if (!db) {
        throw new Error('Firestore unavailable');
    }
 ...
 * Teklifbul Rule v1.0
 */
export async function recordManualStockMovement(input) {
    const db = await getAdminDb();
    if (!db)
        throw new Error('Firestore unavailable');
    const { companyId, userId, createdByName, type, stockId, sku, stockName, unit = 'ADT', locationId, siteId = null, toLocationId = null, qty, unitCost = 0, extras = [], ref = { kind: 'MANUAL', id: '' }, lotNo = null, expiryDate = null, toSiteName = null, confirmNegative = false, } = input;
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (type === 'IN') {
            ...
            if (allowNegativeStock && oldQty < qty && !confirmNegative) {
                const err = new Error('NEGATIVE_STOCK_CONFIRM_REQUIRED');
