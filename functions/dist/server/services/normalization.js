...
export function lowercaseTR(input) {
    if (!input)
        return "";
    let result = input;
...
export function parseNumberTR(value) {
    if (value == null)
        return null;
    ...
    if (hasComma && hasDot) {
        ...
        if (match && !match[3])
            normalized = normalized.replace(NUMBER_THOUSANDS, "");
    }
...
export function detectCurrency(raw) {
    ...
    if (!token)
        return null;
    // exact map first
    if (CURRENCY_MAP[token])
        return CURRENCY_MAP[token];
    // contains fallback (e.g. "odemetry")
    if (token.includes("try") || token.includes("tl"))
        return "TRY";
    if (token.includes("usd") || token.includes("dolar"))
        return "USD";
    if (token.includes("eur") || token.includes("euro"))
        return "EUR";
    if (token.includes("gbp"))
        return "GBP";
    return null;
}
...
export function mapSynonymToField(raw) {
    const key = normalizeKey(raw);
    if (!key)
        return null;
    return FIELD_SYNONYMS[key] || null;
}
export function enrichCandidate(raw) {
