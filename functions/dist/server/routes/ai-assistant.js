...
import { generatePurchaseRequestText, analyzeOfferComparison, autofillRequestFields, validateRequestForErrors, } from '../ai/purchaseAssistantService.js';
...
function shouldSkipWalletPrecheck(provider, isFreeEligible) {
    return isFreeEligible === true || provider === 'groq' || provider === 'free_local';
}
function forceFreeResolved(resolved) {
    return {
        ...resolved,
...
async function tryGetCompanyPurchaseAssistantSettings(params) {
 ...
 */
router.post('/generate-purchase-request', verifyToken, requireAiAccess, async (req, res) => {
    try {
        ...
        let provider = useCompanyFlow ? companySettings.provider : await getUserAIProvider(userId);
        let modelAutoDowngraded = false;
        let resolved = null; // Teklifbul Rule v3.9
        if (useCompanyFlow) {
 ...
 */
router.post('/analyze-offers', verifyToken, requireAiAccess, async (req, res) => {
    try {
 ...
 */
router.post('/autofill-request-fields', verifyToken, requireAiAccess, async (req, res) => {
    try {
 ...
 */
router.post('/validate-request', verifyToken, requireAiAccess, async (req, res) => {
    try {
