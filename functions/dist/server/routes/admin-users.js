 ...
 */
router.get('/users/search', validateRequest({ query: searchUsersQuerySchema }), async (req, res) => {
...
router.get('/users/list', validateRequest({ query: listUsersQuerySchema }), async (req, res) => {
    ...
    try {
            ...
            try {
                ...
                for (let i = 0; i < allUids.length; i += batchSize) {
                    ...
                    const batchPromise = (async () => {
                        try {
                            const getUserIdentifiers = batch.map((uid) => ({ uid }));
        ...
        const companyIds = Array.from(new Set(snapshot.docs.map((d) => {
            const data = d.data() || {};
        ...
        if (companyIds.length > 0) {
            try {
                ...
                await Promise.all(chunks.map(async (chunk) => {
                    ...
                    compSnap.forEach((c) => companyMap.set(c.id, c.data()));
                }));
            }
            catch (err) {
...
router.post('/users/:identifier/plan', validateRequest({
    ...
    try {
        ...
        if (planId !== 'free') {
            ...
            const subscriptionData = {
                ...
                status: 'active',
                billingInterval: planId.includes('yearly') ? 'yearly' : 'monthly',
...
router.get('/users/:id', validateRequest({ params: getUserByIdParamsSchema }), async (req, res) => {
