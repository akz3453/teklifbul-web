 ...
 */
export async function detectTemplate(buffer: Buffer): Promise<TemplateDetectionResult> {
  ...
  try {
    ...
    const customProps = (workbook.properties as any)?.customProperties || [];
    const signatureProp = customProps.find((p: any) => p.name === 'TemplateSignature');
    const versionProp = customProps.find((p: any) => p.name === 'TemplateVersion');
 ...
 */
export function checkVersionCompatibility(templateVersion: string): {
  compatible: boolean;
