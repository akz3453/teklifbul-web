...
async function getCompanyContext(req) {
    ...
    if (!userId)
        return { userId: null, companyId: null, userData: null };
    const db = await getAdminDb();
    if (!db)
        return { userId, companyId: null, userData: null };
...
async function loadPackagesFromFirestore(db) {
    try {
        ...
        return items
            ...
            allowedProviders: Array.isArray(p.allowedProviders) ? p.allowedProviders.map((x) => String(x || '').trim()).filter(Boolean) : [],
            allowedModels: Array.isArray(p.allowedModels) ? p.allowedModels.map((x) => String(x || '').trim()).filter(Boolean) : (p.allowedModels === null ? null : null),
        ...
        logger.warn('ai_token_packages load failed (firestore)', { error: e?.message || e });
        return null;
    }
}
function adaptPackagesFromCodeCatalog() {
 ...
 */
function getProviderKeyFromPackage(pkg) {
    ...
    if (Array.isArray(pkg.allowedProviders) && pkg.allowedProviders.length > 0) {
        ...
        if (provider === 'openai' || provider.startsWith('openai'))
            return 'openai';
        if (provider === 'gemini' || provider.startsWith('gemini'))
            return 'gemini';
        return provider;
    }
...
const router = Router();
router.get('/token-packages', verifyToken, async (req, res) => {
...
router.post('/token-purchases/create', verifyToken, validateRequest({ body: createSchema }), async (req, res) => {
    ...
    try {
        ...
        const allowMock = isMockPurchaseEnabled() && isAdminUser(req.user);
        const db = await getAdminDb();
        if (!db) {
            logger.end();
            return res.status(500).json({ ok: false, error: 'db_unavailable', message: 'Veritabanı bağlantısı kurulamadı.' });
        ...
        if (idempotencyKey) {
            ...
            const hasEntitlements = (Array.isArray(pkg.allowedModels) && pkg.allowedModels.length > 0) ||
                (Array.isArray(pkg.allowedModelPrefixes) && pkg.allowedModelPrefixes.length > 0) ||
                (Array.isArray(pkg.allowedTiers) && pkg.allowedTiers.length > 0);
            const result = await db.runTransaction(async (tx) => {
                ...
                const entitlementSnapshot = hasEntitlements ? {
                    ...
                    allowedModelPrefixes: Array.isArray(pkg.allowedModelPrefixes) ? pkg.allowedModelPrefixes : undefined,
                    allowedTiers: Array.isArray(pkg.allowedTiers) ? pkg.allowedTiers : undefined,
                ...
                tx.set(purchaseRef, {
                    ...
                    packageSnapshot: {
                        ...
                        allowedModelPrefixes: pkg.allowedModelPrefixes,
