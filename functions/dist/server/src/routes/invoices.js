 ...
 */
router.post('/', requirePermission('einvoice.create'), async (req, res) => {
    try {
 ...
 */
router.post('/direct', requirePermission('einvoice.create'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.get('/:id', validate({ params: invoiceIdParamsSchema }), requirePermission('einvoice.view'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 * E-fatura gönder
 */
router.post('/:id/send-efatura', async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.post('/:id/prepare', validate({
    params: invoiceIdParamsSchema,
    body: companyIdBodySchema
}), requirePermission('einvoice.create'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.post('/:id/send', validate({
    params: invoiceIdParamsSchema,
    body: companyIdBodySchema
}), edocSendLimiter, requirePermission('einvoice.send'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.get('/:id/status', validate({ params: invoiceIdParamsSchema }), edocStatusLimiter, requirePermission('einvoice.status'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 * Permission: einvoice.view
 */
router.get('/:id/pdf', validate({ params: invoiceIdParamsSchema }), edocPdfLimiter, requirePermission('einvoice.view'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
...
router.post('/:id/cancel', validate({
    params: invoiceIdParamsSchema,
    body: companyIdBodySchema
}), edocCancelLimiter, requirePermission('einvoice.cancel'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
