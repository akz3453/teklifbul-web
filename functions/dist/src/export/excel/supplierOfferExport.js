 ...
 */
export async function exportSupplierOffer(offer, signal, reportProgress) {
    const report = (p) => {
        if (reportProgress)
            reportProgress(p);
        if (signal)
 ...
 */
function fillHeaderSection(worksheet, offer) {
 ...
 */
function fillLineRow(worksheet, rowNum, line, lineNo) {
    const row = worksheet.getRow(rowNum);
    // A: No
 ...
 */
function fillTotalRow(worksheet, rowNum) {
    const row = worksheet.getRow(rowNum);
    // A: "TOPLAM" etiketi
 ...
 * @param reportProgress - Progress callback (0-100)
 */
export async function exportSupplierOfferBrowser(offerData, signal, reportProgress) {
    const report = (p) => {
        if (reportProgress)
            reportProgress(p);
        if (signal)
