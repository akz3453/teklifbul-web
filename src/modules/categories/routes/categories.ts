...
router.post('/:id/desc', verifyToken, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.post('/suggest', verifyToken, aiSuggestLimiter, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { text } = req.body;
    ...
    try {
      ...
      if (json.categoryId) categoryId = json.categoryId;
    } catch (e) {
...
router.post('/feedback', verifyToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
