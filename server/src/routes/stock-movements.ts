...
router.post(
  '/',
  requireAnyPermission([...MOVEMENT_PERMS]),
  async (req: any, res) => {
    try {
      ...
      if (!allowed) {
        return respondError(
          res,
          Errors.forbidden(`Bu tip stok hareketi için yetkiniz yok: ${specificPerm}`, specificPerm)
        );
      }
      ...
      const result = await recordManualStockMovement({
        ...
        type: type as 'IN' | 'OUT' | 'TRANSFER' | 'ADJUST',
...
router.post(
  '/sku-merge',
  requireAnyPermission(['stock.edit', 'stock.create', 'stock.movements.adjust']),
  async (req: any, res) => {
    try {
      const userId = req.user?.uid;
      if (!userId) {
