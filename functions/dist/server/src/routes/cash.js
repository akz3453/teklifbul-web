...
import { createCashAccount, updateCashAccount, getCashAccounts, recordCashTransaction, getCashTransactions, transferBetweenCashAccounts, deleteCashAccount, getDefaultCashAccount } from '../services/cashService.js';
...
async function requireTrustedCompanyId(req, res) {
 ...
 * Kasa listesi
 */
router.get('/', requirePermission('cash.view'), async (req, res) => {
    try {
 ...
 * Varsayılan kasa
 */
router.get('/default', requirePermission('cash.view'), async (req, res) => {
    try {
        const companyId = await requireTrustedCompanyId(req, res);
        if (!companyId)
            return;
 ...
 * Kasa detayı
 */
router.get('/:id', requirePermission('cash.view'), async (req, res) => {
    try {
        const { id } = req.params;
        const companyId = await requireTrustedCompanyId(req, res);
        if (!companyId)
 ...
 * Yeni kasa oluştur
 */
router.post('/', validate({ body: createCashAccountSchema.shape.body }), requirePermission('cash.create'), async (req, res) => {
    try {
 ...
 * Kasa güncelle
 */
router.put('/:id', validate({ params: updateCashAccountSchema.shape.params, body: updateCashAccountSchema.shape.body }), requirePermission('cash.edit'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.delete('/:id', requirePermission('cash.delete'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 * Kasa hareketleri
 */
router.get('/:id/transactions', requirePermission('cash.view'), async (req, res) => {
    try {
        const { id } = req.params;
        const companyId = await requireTrustedCompanyId(req, res);
        if (!companyId)
 ...
 * Kasa hareketi ekle
 */
router.post('/:id/transactions', validate({ params: cashTransactionSchema.shape.params, body: cashTransactionSchema.shape.body }), requirePermission('cash.create'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.post('/transfer', validate({ body: transferSchema.shape.body }), requirePermission('cash.create'), async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
