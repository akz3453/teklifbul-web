...
import { getUserAllTokenPacks } from '../services/aiTokenPackService.js';
 ...
 */
router.post('/initiate-payment', verifyToken, requireAiAccess, validateRequest({
    body: z.object({
        packId: z.string()
    })
}), async (req, res) => {
    ...
    try {
        ...
        const paymentIntent = await createPaymentIntentRecord({
            userId,
            planId: `token_pack_${packId}`, // Özel planId formatı
            amount: pack.price,
