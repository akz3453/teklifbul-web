...
import { assertValidStatusTransition } from './saleStateMachine.js';
 ...
 */
export async function approveSale(saleId, userId, companyId, requestId) {
    ...
    return await db.runTransaction(async (transaction) => {
        ...
        const lockExpiresAt = new Date(Date.now() + 30000);
        ...
        const stockResult = await createStockMovements(sale.items || [], sale.companyId, saleId, userId, false // allowNegativeStock - şirket ayarından kontrol edilecek
        );
 ...
 */
export async function cancelSale(saleId, userId, companyId, reason) {
    const db = await getAdminDb();
    if (!db) {
        throw new Error('Firestore unavailable');
    }
    ...
    if (sale.stockMovementCreated && sale.stockMovementIds && sale.stockMovementIds.length > 0) {
        try {
            const reversalMovementIds = await createReversalMovements(sale.items || [], sale.stockMovementIds, sale.companyId, saleId, userId);
 ...
 */
export async function rejectSale(saleId, userId, companyId, reason) {
    const db = await getAdminDb();
    if (!db) {
        throw new Error('Firestore unavailable');
    }
