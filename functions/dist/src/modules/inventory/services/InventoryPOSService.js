...
export class InventoryPOSService {
     ...
     */
    static async processSale(payload, attempt = 1) {
        const MAX_RETRIES = 3;
        ...
        try {
            ...
            if (attempt === 1 && payload.idempotency_key) {
                ...
                if (idempRes.rows.length > 0) {
                    ...
                    return idempRes.rows[0].response_body;
                }
            }
        ...
        `;
                ...
                if (bRes.rows.length === 0)
                    throw new Error('Alias_Barcode ile eslesen gecerli ve stoklu Batch bulunamadi.');
                const b = bRes.rows[0];
                ...
                if (diffDays < 0 && !b.allow_expired_sale)
                    throw new Error('HARD BLOCK: Tarihi gecmis urun satisi engellendi.');
        ...
        `;
                ...
                for (const row of bRes.rows) {
                    ...
                    if (diffDays >= 0 && diffDays <= row.critical_skt_threshold_days) {
                        ...
                        if (diffDays < 5)
                            discount_rate = Math.max(discount_rate, 40);
                        else if (diffDays < 10)
                            discount_rate = Math.max(discount_rate, 20);
                    }
                }
