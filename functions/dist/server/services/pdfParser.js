...
let __pdfParseFn = null;
async function getPdfParse() {
    if (__pdfParseFn)
        return __pdfParseFn;
    // Prefer dynamic import to let Node/tsx resolve ESM/CJS shapes
    try {
        const sub = await import('pdf-parse/lib/pdf-parse.js');
...
async function parseWithPdfJs(buf) {
    ...
    if (!pdfjs?.getDocument) {
        throw new Error(`[pdfParser] Unable to load pdfjs-dist fallback: ${lastErr?.message || lastErr}`);
    }
...
function pickCurrencyFromText(text) {
...
// --- Core -----------------------------------------------------------------
export async function previewPdf(buf) {
    ...
    const requester = (text.match(RX.requester)?.[2] || lines[1] || "").trim() || null;
    ...
    const dedupKey = (i) => `${i.itemName}__${i.qty ?? ""}__${i.unit ?? ""}`;
    const seen = new Set();
    ...
    if (uniqueItems.length === 0) {
        warnings.push("PDF metninden tablo tespit edilemedi. PN/SN ya da sütunlar arası çift boşluk/sekme kullanımı önerilir. Excel formatı daha sağlıklıdır.");
    }
    ...
    // güven puanı
    const confidence = uniqueItems.length >= 3 ? 85 :
