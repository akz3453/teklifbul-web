...
import { createCustomer, updateCustomer, archiveCustomer } from '../services/customerService.js';
...
router.get('/:id/movements', async (req, res) => {
    try {
        ...
        const customerId = req.params.id;
        const startDate = req.query.startDate && req.query.startDate !== '' ? new Date(req.query.startDate) : null;
        const endDate = req.query.endDate && req.query.endDate !== '' ? new Date(req.query.endDate) : null;
        const db = await getAdminDb();
        if (!db) {
            return res.status(500).json({ ok: false, error: 'Firestore unavailable' });
        }
 ...
 */
router.get('/:id/extre', async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ ok: false, error: 'Unauthorized' });
        ...
        const ALL_EXPORT_TYPES = ['customerInfo', 'sale', 'invoice', 'deliveryNote', 'payment'];
        const rawTypes = String(req.query.types || '')
            .split(',')
            .map((t) => t.trim())
            .filter(Boolean);
        const selectedTypes = new Set((rawTypes.length > 0 ? rawTypes : [...ALL_EXPORT_TYPES]).filter((t) => ALL_EXPORT_TYPES.includes(t)));
        ...
        if (includePayment) {
            ...
            try {
                ...
                fallbackSnap.docs.forEach((doc) => {
                    ...
                    if (startDate && txDate && txDate < startDate)
                        return;
                    if (endDate && txDate) {
                        const endDateWithTime = new Date(endDate);
                        endDateWithTime.setHours(23, 59, 59, 999);
                        if (txDate > endDateWithTime)
                            return;
                    }
                    movements.push({
                        id: doc.id,
        ...
        if (includeSale || includeInvoice || includeDeliveryNote || includePayment) {
            ...
            movements.forEach(m => {
                ...
                if (m.type === 'sale') {
                    const saleMap = { saved: 'Faturalanmadı', invoiced: 'Faturalandı', cancelled: 'İptal', archived: 'Arşiv' };
