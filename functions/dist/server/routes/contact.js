...
async function resolveAdminUids(db) {
    ...
    for (const email of emails) {
        try {
            ...
            logger.warn('Admin uid sorgusu (email) başarısız', { email, error: e?.message });
        }
    }
    return [...uids];
}
async function notifyAdmins(db, messageId, payload) {
    ...
    const preview = payload.message.length > 120
        ...
        : payload.message;
    const writes = adminUids.map((uid) => db.collection('notifications').add({
        userId: uid,
        ...
        logger.warn('Admin bildirim yazılamadı', { uid, error: err?.message });
    }));
...
contactAdminRouter.get('/contact-messages', async (req, res) => {
    ...
    try {
        ...
        let queryRef = db.collection(CONTACT_COLLECTION).orderBy('createdAt', 'desc').limit(limit);
        if (status !== 'all') {
        ...
        if (search) {
            messages = messages.filter((m) => [m.name, m.email, m.company, m.phone, m.message]
                .filter(Boolean)
...
contactAdminRouter.patch('/contact-messages/:id', async (req, res) => {
    ...
    try {
        ...
        if (parsed.data.adminNote !== undefined)
            patch.adminNote = parsed.data.adminNote;
