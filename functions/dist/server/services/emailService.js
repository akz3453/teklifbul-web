...
function isCircuitOpen(provider) {
    ...
    if (!circuit)
        return true; // Unknown provider = blocked
...
export async function checkEmailQuota(userId, companyId) {
    try {
        const db = await getAdminDb();
        if (!db) {
...
export async function incrementEmailUsage(userId, companyId, provider) {
    try {
        const db = await getAdminDb();
        if (!db)
            return;
...
export async function getEmailQuotaStatus(userId, companyId) {
    const planType = await getUserPlanType(userId);
    const limits = QUOTA_LIMITS[planType];
    const db = await getAdminDb();
    if (!db) {
...
async function sendViaBrevo(to, subject, html, from, attachments) {
    try {
...
async function sendViaMailjet(to, subject, html, from, attachments) {
    try {
 ...
 */
export function isBrandedEmailReady() {
...
// ============================================
export async function sendSupplierQuoteEmail(payload) {
