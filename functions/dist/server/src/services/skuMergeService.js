...
export async function mergeSkusAdmin(params) {
    ...
    const sourceBalances = sourceBalancesSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
    const targetBalances = targetBalancesSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
    const ops = [];
    for (const sourceBalance of sourceBalances) {
        ...
        if (targetBalance) {
            ...
            ops.push((batch) => batch.set(balanceRef, {
                companyId,
                sku: targetSku,
                locationId,
        ...
        ops.push((batch) => batch.delete(db.collection('stock_balances').doc(getBalanceDocId(companyId, sourceSku, locationId))));
    }
    for (const movementDoc of sourceMovementsSnap.docs) {
        ops.push((batch) => batch.update(movementDoc.ref, {
            sku: targetSku,
            stockId: targetStock.id,
    ...
    const newLastPurchasePrice = Math.max(Number(sourceStock.lastPurchasePrice) || 0, Number(targetStock.lastPurchasePrice) || 0);
    const newSalePrice = Math.max(Number(sourceStock.salePrice) || 0, Number(targetStock.salePrice) || 0);
    ops.push((batch) => batch.update(db.collection('stocks').doc(targetStock.id), {
        avgCost: newAvgCost,
        lastPurchasePrice: newLastPurchasePrice,
        salePrice: newSalePrice,
        updatedAt: FieldValue.serverTimestamp(),
    }));
    ops.push((batch) => batch.update(db.collection('stocks').doc(sourceStock.id), {
        merged: true,
        mergedTo: targetSku,
        mergedAt: FieldValue.serverTimestamp(),
        mergedBy: userId,
