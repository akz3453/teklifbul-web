 ...
 */
router.post('/', clientErrorIngestLimiter, async (req, res) => {
    try {
        ...
        if (!validationResult.success) {
            ...
            return respondError(res, Errors.validation(validationResult.error.errors.map(e => e.message).join(', ')));
        }
        ...
        if (!db) {
            ...
            return respondError(res, Errors.internal('Hata kaydedilemedi: Firestore kullanılamıyor'));
        }
        ...
        logger.end();
        return respondError(res, Errors.internal('Hata kaydedilemedi'));
    }
});
/**
 ...
 */
router.get('/', verifyToken, requirePermission('admin.errors.view'), async (req, res) => {
    try {
        ...
        if (!db) {
            ...
            return respondError(res, Errors.internal('Hatalar listelenemedi: Firestore kullanılamıyor'));
        }
        // Build query
        ...
        logger.end();
        return respondError(res, Errors.internal(`Hatalar listelenemedi: ${error.message}`));
    }
});
