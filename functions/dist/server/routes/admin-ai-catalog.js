...
const SUGGESTED_MODELS_CANDIDATES = [
 ...
 */
router.get('/ai-model-suggestions', async (req, res) => {
    ...
    try {
        ...
        const existingKeys = new Set(snap.docs.map((d) => {
            const data = d.data() || {};
 ...
 * Create new AI model
 */
router.post('/ai-models', validateRequest({ body: aiModelSchema }), async (req, res) => {
    ...
    try {
        ...
        const { provider, model, label, isActive, freeEligible, sort, inputCostPer1MTokensUSD, outputCostPer1MTokensUSD, costPer1kTokensUSD, currency } = req.body;
        ...
        const blendedCostPer1k = normalizedInputPer1M > 0 || normalizedOutputPer1M > 0
 ...
 * Update AI model
 */
router.patch('/ai-models/:id', validateRequest({
...
function computeSuggestedSalePriceTRY(purchaseCostTRY, marginPercent = 20, vatPercent = 20) {
...
async function fetchLiveUsdTry(req) {
    try {
 ...
 */
router.get('/ai-pricing-config', async (_req, res) => {
    try {
        const db = await getAdminDb();
        if (!db)
            return res.status(500).json({ error: 'DB_UNAVAILABLE', message: 'Veritabanı bağlantısı kurulamadı' });
 ...
 */
router.post('/ai-pricing-config', validateRequest({ body: pricingConfigSchema }), async (req, res) => {
    try {
        const db = await getAdminDb();
        if (!db)
            return res.status(500).json({ error: 'DB_UNAVAILABLE', message: 'Veritabanı bağlantısı kurulamadı' });
 ...
 */
router.get('/ai-token-packages', async (req, res) => {
 ...
 */
router.post('/ai-token-packages', validateRequest({ body: tokenPackageSchema }), async (req, res) => {
 ...
 */
router.post('/ai-token-packages/reprice-from-model-costs', validateRequest({ body: repricePackagesSchema.optional() }), async (req, res) => {
 ...
 */
router.get('/ai-price-watch', async (_req, res) => {
    try {
        const db = await getAdminDb();
        if (!db)
            return res.status(500).json({ error: 'DB_UNAVAILABLE', message: 'Veritabanı bağlantısı kurulamadı' });
        ...
        snap.docs.forEach((doc) => {
            ...
            if (!provider || !model || d.isActive === false)
                return;
            ...
            const updatedAtMs = typeof d.updatedAtMs === 'number' ? d.updatedAtMs :
 ...
 */
router.patch('/ai-token-packages/:id', validateRequest({
 ...
 */
router.delete('/ai-token-packages/:id', async (req, res) => {
 ...
 */
router.get('/upgrade-leads', verifyToken, requireAdmin, async (req, res) => {
    try {
