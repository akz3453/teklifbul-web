 ...
 */
export async function resolveGroupMembers(groupIds: string[], uid: string): Promise<string[]> {
  if (!groupIds.length || !uid) return [];
  ...
  try {
    ...
    const categorySlugs = categoryArray.map(cat => 
