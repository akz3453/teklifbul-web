...
import { initiatePayment, handlePaymentWebhook } from '../services/paymentsService.js';
...
router.post('/initiate', verifyToken, requireEmailVerified, validateRequest({ body: initiatePaymentSchema }), async (req, res) => {
    try {
        if (!req.user) {
