 ...
 */
router.post('/:id/process-to-stock', requirePermission('stock.movements.in'), async (req, res) => {
    ...
    try {
        ...
        const result = await db.runTransaction(async (tx) => {
            ...
            if (!incomingSnap.exists) {
                const e = new Error('Gelen belge bulunamadi');
                e.statusCode = 404;
                throw e;
            }
            const incoming = incomingSnap.data();
            if (incoming.companyId !== companyId) {
                const e = new Error('Belge bu sirkete ait degil');
                e.statusCode = 403;
                throw e;
            }
            if (incoming.status === 'processed') {
                const e = new Error('Belge zaten stoga islenmis');
                e.statusCode = 409;
                throw e;
            }
            const items = Array.isArray(incoming.items) ? incoming.items : [];
            if (items.length === 0) {
                const e = new Error('Belgede kalem bulunamadi');
                e.statusCode = 400;
                throw e;
            }
            if (items.length > MAX_ITEMS_PER_TX) {
                const e = new Error(`Bir transaction icin azami ${MAX_ITEMS_PER_TX} kalem isleyebiliriz. Belgeyi bolun.`);
                e.statusCode = 400;
                throw e;
            }
            const unmapped = items.filter((it) => !it.mappedInternalStockId);
            if (unmapped.length > 0) {
                const e = new Error(`${unmapped.length} kalem hala eslenmedi: ${unmapped.map((u) => u.name).join(', ')}`);
                e.statusCode = 400;
                throw e;
            }
            ...
            for (let i = 0; i < stockSnaps.length; i++) {
                if (!stockSnaps[i].exists) {
                    const e = new Error(`Eslenen stok bulunamadi: ${items[i].mappedInternalStockId}`);
                    e.statusCode = 400;
                    throw e;
                }
            }
            ...
            for (let i = 0; i < items.length; i++) {
                const stockData = stockSnaps[i].data();
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                if (finalQty <= 0) {
                    const e = new Error(`Gecersiz miktar: ${item.name}`);
                    e.statusCode = 400;
                    throw e;
                }
