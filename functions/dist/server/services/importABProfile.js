 ...
 */
export async function parseTwoSheetProfile(fileOrArrayBuffer) {
    try {
        // Buffer'a dönüştür
        let buffer;
        ...
        const demandSheet = workbook.worksheets.find(s => (s.name || '').toLowerCase().includes('talep bilgileri') ||
            (s.name || '').toLowerCase().includes('talep')) || workbook.worksheets[0];
        if (!demandSheet) {
            throw new Error('parse_error: Talep Bilgileri sayfası bulunamadı');
        }
        // Kalemler sayfasını bul
        const itemsSheet = workbook.worksheets.find(s => (s.name || '').toLowerCase() === 'kalemler' ||
            (s.name || '').toLowerCase().includes('kalem')) || workbook.worksheets[1];
        // --- Talep Bilgileri ---
        const demandMapping = {
            'başlık': 'title',
            'şantiye': 'siteName',
        ...
        for (let row = 1; row <= Math.min(50, demandSheet.rowCount); row++) {
            ...
            if (!headerRaw || !valueRaw)
                continue;
            // Başlığı normalize et
            const normalizedKey = normalizeTr(headerRaw);
            // Eşleştirme ara
            const match = Object.keys(demandMapping).find(k => normalizedKey.startsWith(k) || normalizedKey.includes(k));
            if (match) {
        ...
        if (itemsSheet) {
            ...
            for (let row = headerRow + 1; row <= Math.min(itemsSheet.rowCount, headerRow + 200); row++) {
                ...
                // "Not:" satırını atla
                if (!itemName || normalizeTr(itemName).startsWith('not'))
                    continue;
 ...
 */
export async function parseTwoColumnProfile(fileOrArrayBuffer) {
    try {
        // Buffer'a dönüştür
        let buffer;
        if (fileOrArrayBuffer instanceof File) {
