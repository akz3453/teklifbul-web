...
import type { AIProvider } from '../services/userService.js';
 ...
 */
router.post('/sync-company-context', verifyToken, async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    if ((!resolvedCompanyId || resolvedCompanyId.startsWith('solo-')) && userData.companyCode) {
      ...
      if (!joinRequestsSnapshot.empty) {
        ...
        const acceptedRequest =
          allRequests.find((r: any) => r.status === 'accepted' || r.status === 'approved') || null;
        const pendingRequest = allRequests.find((r: any) => r.status === 'pending') || null;
...
router.post('/ai-provider', verifyToken, validateRequest({ 
  body: z.object({ 
    provider: z.enum(['gemini', 'openai']).refine(
      (val) => val === 'gemini' || val === 'openai',
      { message: 'Geçersiz provider değeri. "gemini" veya "openai" olmalıdır.' }
    )
  })
