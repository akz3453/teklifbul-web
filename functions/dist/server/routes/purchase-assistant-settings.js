...
function computeModelAccessHints(params) {
    ...
    const activeCatalog = (catalog || [])
        .filter((m) => m && m.isActive !== false && m.provider !== 'ollama');
    const availableSet = new Set((availableModels || []).map((m) => `${String(m.provider || '')}::${String(m.model || '')}`));
    const lockedModels = activeCatalog
        .filter((m) => !availableSet.has(`${String(m.provider || '')}::${String(m.model || '')}`))
        .map((m) => ({
...
router.get('/', verifyToken, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            logger.warn('Company plan read failed, falling back to account summary', { error: e?.message || e });
        }
        if (!planId && userId) {
        ...
        const catalogModels = catalogSnap.docs.map((d) => ({ ...(d.data() || {}) }));
...
router.post('/', verifyToken, validateRequest({ body: postSchema }), async (req, res) => {
