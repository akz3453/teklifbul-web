 ...
 */
export function hasCriticalFieldChanges(changedFields: string[], before: Record<string, any>, after: Record<string, any>): boolean {
 ...
 */
export function calculateDiff(before: Record<string, any>, after: Record<string, any>): {
  changedFields: string[];
  before: Record<string, any>;
  after: Record<string, any>;
} {
 ...
 */
export async function createSaleRevision({
  saleId,
  companyId,
  userId,
  userEmail,
...
}): Promise<{ revisionId: string; saleVersion: number }> {
  ...
  return await db.runTransaction(async (transaction) => {
    ...
    const currentSale = saleDoc.data() as any;
    ...
    const revision: SaleRevision = {
      ...
      editedAt: FieldValue.serverTimestamp() as Timestamp,
      editedBy: {
        userId,
 ...
 * Get sale revisions
 */
export async function getSaleRevisions(
  saleId: string,
  companyId: string
): Promise<SaleRevision[]> {
  const db = await getAdminDb();
  if (!db) {
    throw new Error('Firestore unavailable');
  }
