 ...
 */
export async function injectVBA(
  excelBuffer: Buffer | ArrayBuffer,
  worksheetName: string = 'Teklif'
): Promise<Buffer> {
...
End Sub`;
    ...
    zip.file('VBA_CODE_REFERENCE.txt', 
      `VBA KODU - ${worksheetName} Sayfasına Ekleyin\n\n` +
    ...
    return excelBuffer instanceof Buffer ? excelBuffer : Buffer.from(excelBuffer as ArrayBuffer);
  }
}
