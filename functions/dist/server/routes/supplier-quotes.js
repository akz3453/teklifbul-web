...
import { checkEmailQuota, incrementEmailUsage, getEmailQuotaStatus, sendSupplierQuoteEmail, getProviderHealth } from '../services/emailService.js';
import { generateQuoteToken, validateToken, getRequestByToken, submitQuote, getQuotesForRequest, updateQuoteStatus, getTokensForRequest } from '../services/supplierQuoteService.js';
...
router.post('/submit/:token', async (req, res) => {
    try {
        ...
        // Submit the quote
        const result = await submitQuote(token, supplier, items, summary, notes, attachmentUrl);
        if (!result.success) {
            res.status(400).json({ success: false, error: result.error });
            return;
        }
 ...
 */
router.get('/list/:requestId', verifyToken, async (req, res) => {
    try {
 ...
 */
router.get('/tokens/:requestId', verifyToken, async (req, res) => {
    try {
        const { companyId } = await getAuthInfo(req);
        const requestId = req.params.requestId;
        if (!companyId) {
 ...
 */
router.patch('/:quoteId/status', verifyToken, async (req, res) => {
    try {
        const { companyId } = await getAuthInfo(req);
        ...
        const result = await updateQuoteStatus(quoteId, companyId, status);
        if (!result.success) {
            res.status(400).json({ success: false, error: result.error });
            return;
        }
