...
async function getAuthorizedWaybillAccess(db, demandId, userId, companyId) {
...
router.post('/:demandId/upload', verifyToken, requirePremium, validateRequest({ params: uploadParamsSchema, body: uploadBodySchema }), upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        // CHECK AUTHORIZATION
        const auth = await getAuthorizedWaybillAccess(db, demandId, userId, companyId);
        if (!auth.authorized) {
        ...
        const fileUrl = await uploadFile(bucket, storagePath, req.file.buffer, req.file.mimetype || 'application/octet-stream', { uploadedBy: userId, demandId, role });
...
router.get('/:demandId', verifyToken, requirePremium, validateRequest({ params: uploadParamsSchema }), async (req, res) => {
    ...
    try {
        ...
        const bucket = await getAdminStorage();
        const resignList = async (items) => {
            if (!Array.isArray(items) || !bucket)
                return items || [];
