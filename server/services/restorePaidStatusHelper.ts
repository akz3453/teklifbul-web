...
export type RestorePaidStatusResult = {
...
export async function computeRestorePaidStatus(params: {
  ...
  settings: any;
  planId: string | null;
}): Promise<RestorePaidStatusResult> {
  ...
  if (!resolvedPlanId && userId) {
    try {
      ...
      logger.warn('Failed to get account subscription summary', { error: (e as any)?.message || e });
    }
  }
  ...
  let suggestedPaid: { provider: string; model: string; providerKey?: string } | null = null;
  if (hasPaidCandidate) {
  ...
  let reason: 'FORCED_FREE_MODE' | 'NO_PAID_MODEL' | 'ALREADY_PAID' | 'NOT_ENTITLED' | 'NO_FUNDS_FOR_PAID' | 'OK' = 'OK';
