 ...
 */
function attachRequiredProviderKey(mapping, existingMeta) {
    if (!mapping.meta) {
        mapping.meta = {};
    }
 ...
 */
export function mapAiError(code, existingMeta) {
    switch (code) {
        case AI_ERROR_CODES.INSUFFICIENT_TOKENS:
            return attachRequiredProviderKey({ status: 402, userMessage: 'Yetersiz token. Lütfen yeni bir paket satın alın.', code }, existingMeta);
        ...
        case AI_ERROR_CODES.INVALID_PROVIDER:
            ...
            return attachRequiredProviderKey({ status: 400, userMessage: 'Geçersiz provider. Paket providerKey içermelidir.', code }, existingMeta);
 ...
 */
export function mapAiDisabledError(reason, existingMeta) {
