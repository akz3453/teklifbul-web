 ...
 */
router.post('/satin-alma', verifyToken, requirePremium, validateRequest({ body: satinAlmaSchema }), async (req: AuthenticatedRequest, res) => {
  try {
...
router.post('/category-guard', optionalVerifyToken, validateRequest({ body: categoryGuardSchema }), async (req: OptionalAuthRequest, res) => {
  try {
    ...
    const trimmedCategories = categories
      .map((cat: string) => (cat || '').toString().trim())
      .filter(Boolean);
