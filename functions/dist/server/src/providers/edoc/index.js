 ...
 */
export async function getEdocProvider(companyId, req) {
    ...
    if (req) {
        const reqCached = getFromReqCache(req, companyCacheKey);
        if (reqCached !== null) {
    ...
    // TTL cache kontrolü
    if (!company) {
        const ttlCached = ttlCache.get(companyCacheKey);
        if (ttlCached !== null) {
            if (process.env.DEBUG_CACHE === 'true') {
    ...
    if (credentialsRef) {
        ...
        // Request-scope cache kontrolü
        if (req) {
            const reqCached = getFromReqCache(req, credsCacheKey);
            if (reqCached !== null) {
                if (process.env.DEBUG_CACHE === 'true') {
        ...
        // TTL cache kontrolü
        if (!credentials) {
            const ttlCached = ttlCache.get(credsCacheKey);
            if (ttlCached !== null) {
                if (process.env.DEBUG_CACHE === 'true') {
