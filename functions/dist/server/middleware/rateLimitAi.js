...
async function getTrustedCompanyKey(req) {
    try {
        ...
        if (companyId)
            return `company:${companyId}:ai`;
    }
    catch (e) {
        logger.warn('rateLimitAi: trusted company resolve failed', e);
    }
    if (req.user?.uid)
        return `user:${req.user.uid}:ai`;
    return null;
}
async function getPlanId(req) {
    try {
        const { getCompanyIdFromRequest } = await import('../src/services/permissionService.js');
        const companyId = await getCompanyIdFromRequest(req);
        if (companyId) {
...
async function consumeDistributedWindow(key, limit, windowMs, now) {
