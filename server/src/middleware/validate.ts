...
import { Errors, makeError } from '../errors/errorCatalog.js';
    ...
    try {
      ...
      if (schemas.params) {
        ...
        if (!paramsResult.success) {
          ...
          return respondError(
            res,
            Errors.validation('Geçersiz istek parametreleri', details)
          );
        }
      ...
      if (schemas.query) {
        ...
        if (!queryResult.success) {
          ...
          return respondError(
            res,
            Errors.validation('Geçersiz sorgu parametreleri', details)
          );
        }
        ...
        try {
          ...
          Object.assign(req.query as any, queryResult.data);
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      if (schemas.body) {
        ...
        if (!bodyResult.success) {
          ...
          return respondError(
            res,
            Errors.validation('Geçersiz istek gövdesi', details)
          );
        }
      ...
      return respondError(
        res,
        Errors.internal('Validation middleware hatası')
      );
    }
  };
}
