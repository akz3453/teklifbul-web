...
import { AI_RESOLVE_BY, AI_RESOLVE_REASON } from '../constants/aiMeta.js';
...
export async function resolveCompanyAiModelForRequest(args) {
    ...
    try {
        ...
        const selectedModelObj = availableModels.find(m => m.provider === selectedProvider && m.model === selectedModel);
        // Teklifbul Rule v1.0 — Ücretsiz model her zaman hemen kullanılır; ücretli EXACT için cüzdan şart
        let paidExactUnusable = null;
        if (selectedModelObj) {
        ...
        if (!paidExactUnusable && planFlags.isPremiumPlus && hasPaidCandidate && restorePaidStatus.suggestedPaid) {
            ...
            const verified = availableModels.find(m => m.provider === suggested.provider && m.model === suggested.model);
            if (verified) {
        ...
        if (freeCandidates.length > 0) {
            ...
            let requiredProviderKey = undefined;
            ...
            const suggestedPaid = paidExactUnusable
                ? {
