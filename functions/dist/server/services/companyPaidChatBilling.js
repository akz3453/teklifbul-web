...
import { AI_HOLD_BILLING_STATUS, AI_HOLD_PROVIDER_STATUS, AI_TOKEN_HOLD_STATUS, captureCompanyTokensTransactional, isHoldProviderSuccessPendingCapture, markCompanyAiHoldProviderOutcome, releaseCompanyTokensTransactional, reserveCompanyTokensTransactional, } from './companyAiWalletService.js';
...
export async function runCompanyPaidChatTurn(params) {
    const { companyId, userId, provider, model, requestId, estimatedTokens, invokeProvider, meta, } = params;
    ...
    try {
        ...
        logger.end();
        if (code === AI_ERROR_CODES.INSUFFICIENT_TOKENS ||
    ...
    if (reserveResult.replay) {
        ...
        if (pendingCapture) {
            const actualTokens = reserveResult.actualTokens && reserveResult.actualTokens > 0
        ...
        const replayCode = String(reserveResult.billingStatus || '') === AI_HOLD_BILLING_STATUS.NEEDS_RECONCILIATION
    ...
    try {
        ...
        const providerStatus = classifyProviderError(providerError) === 'timeout'
