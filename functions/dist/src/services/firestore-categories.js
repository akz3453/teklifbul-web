...
import { collection, query, getDocs, doc, getDoc, addDoc, orderBy, limit } from 'firebase/firestore';
...
export async function getCategories(options) {
    ...
    try {
        ...
        if (search) {
            ...
            const categories = allCategories.filter(cat => cat.name.toLowerCase().includes(searchLower) ||
            ...
            const q = query(categoriesRef, orderBy('name', 'asc'), limit(size));
            const snapshot = await getDocs(q);
...
export async function suggestCategory(text) {
    ...
    try {
        ...
        const topCategories = Array.from(categoryScores.entries())
            ...
            .slice(0, 5);
        const suggestions = await Promise.all(topCategories.map(async ([catId, data]) => {
