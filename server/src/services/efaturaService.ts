...
import { validateEInvoiceData, EFaturaScenario, EFaturaType } from './efaturaValidator.js';

import { logAuditEvent } from './auditService.js';
import { getEdocProvider } from '../providers/edoc/index.js';
import { createPurchaseMovements } from './stockService.js';
import { IncomingInvoice } from '../providers/edoc/types.js';
...
export async function sendEInvoice(
  ...
  const eInvoiceData = {
    ...
    scenario: 'SALE' as EFaturaScenario,
    type: 'SATIS' as EFaturaType,
 ...
 */
export async function sendEDeliveryNote(
 ...
 */
export async function checkIncomingInvoices(
  companyId: string,
  userId: string
  ...
  let incomingInvoices: IncomingInvoice[] = [];
  try {
  ...
  for (const inv of incomingInvoices) {
    try {
      ...
      await incomingRef.set({
        ...
        items: inv.items.map((item: any, itemIndex: number) => ({
