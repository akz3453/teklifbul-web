...
import { resolveTrustedCompanyIdAsync, userBelongsToCompanyAsync, } from '../../utils/companyAccess.js';
 ...
 */
function loadRolePermissionsTemplate() {
 ...
 */
async function getUserCompanyRole(userId, companyId, req) {
    try {
        // Request-scope cache kontrolü
        if (req) {
            const cacheKey = CacheKeys.membership(companyId, userId);
            const cached = getFromReqCache(req, cacheKey);
            if (cached !== null) {
                if (process.env.DEBUG_CACHE === 'true') {
        ...
        const ttlCached = ttlCache.get(ttlCacheKey);
        if (ttlCached !== null) {
            if (process.env.DEBUG_CACHE === 'true') {
 ...
 */
function isIsverenRole(roleCode) {
...
function isSalesReadPerm(permKey) {
 ...
 */
export async function hasPermission(userId, companyId, permKey, req) {
    try {
 ...
 * Teklifbul Rule v1.0 - Request-scope cache desteği
 */
export async function hasAnyPermission(userId, companyId, permKeys, req) {
 ...
 */
export async function getCompanyIdFromRequest(req) {
    ...
    const bodyCompanyId = req.body && typeof req.body.companyId === 'string'
 ...
 */
export function getCompanyIdFromRequestSync(_req) {
    return null;
}
