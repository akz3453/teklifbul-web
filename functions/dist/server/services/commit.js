...
const DB = { demands: new Map(), bySATFK: new Map() };
function genSATFK() {
    const d = new Date();
...
export async function commitDemand({ demand, items }) {
    let satfk = (demand.satfk || '').toString().trim();
    if (satfk) {
        // Check in DB or Firestore
        if (DB.bySATFK.has(satfk))
            throw new Error('Bu SATFK daha önce kullanılmış');
        if (db) {
            try {
                ...
                console.log('[Firestore] SATFK check failed, using mock DB', err?.message);
                db = null;
            }
        }
    }
    ...
    if (db) {
        try {
            ...
            console.info('[Firestore] write failed, using mock DB', err?.message);
            db = null;
