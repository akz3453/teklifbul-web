 ...
 */
async function getUsdTryRate(db) {
    try {
 ...
 */
function calculateCostUSD(tokens, costPer1k) {
    return (tokens / 1000) * costPer1k;
}
/**
 * Calculate usage summary for a company (reuse usage-report logic)
 */
async function calculateUsageSummary(db, companyId, days, costMap, catalog, usdTryRate) {
    const now = new Date();
    ...
    const byModel = Array.from(byModelMap.values()).map(m => ({
        ...
        costSource: (m.costFromLedger > 0 && m.costEstimated === 0) ? 'ledger' :
            (m.costFromLedger > 0 && m.costEstimated > 0) ? 'ledger' : // Mixed, but prefer ledger
                'estimated',
    ...
    const byRoute = Array.from(byRouteMap.values()).map(r => ({
        ...
        costSource: (r.costFromLedger > 0 && r.costEstimated === 0) ? 'ledger' :
            (r.costFromLedger > 0 && r.costEstimated > 0) ? 'ledger' : // Mixed, but prefer ledger
                'estimated',
 ...
 * GET /api/admin/company-profit-detail?companyId=...&month=YYYY-MM
 */
router.get('/company-profit-detail', async (req, res) => {
    try {
        ...
        let forcedFreeModeReason = null;
        let dailyPaidTokenCap = null;
        let dailyPaidTokenCapReason = null;
        try {
