 ...
 */
export async function analyzeError(errorData: any, userId?: string): Promise<ErrorAnalysis> {
...
export async function analyzeErrorsBatch(
  ...
  try {
    ...
    const validErrors = errorDocs.filter((e): e is { id: string; data: any } => e !== null);
    ...
    const results: Array<{ errorId: string; analysis: ErrorAnalysis }> = [];
