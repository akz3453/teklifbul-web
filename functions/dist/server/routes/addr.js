...
router.get('/streets', async (req, res) => {
    try {
        ...
        try {
            ...
            const normalizedStreets = streets.map((s) => ({
                id: s.id || s.neighborhoodId || null,
                name: s.name || '',
                postalCode: s.postalCode || s.postal_code || null
            })).filter((s) => s.name); // Boş isimleri filtrele
            return res.json({
                ok: true,
 ...
 */
router.get('/resolve-ids', async (req, res) => {
    try {
        ...
        try {
            ...
            const neighborhood = neighborhoods.find((n) => normalize(n.name) === normalizedNeighborhoodName);
            if (!neighborhood) {
