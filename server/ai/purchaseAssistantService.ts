...
import { sendChat, type AIProvider, type ChatMessage } from './index.js';
...
export interface GeneratePurchaseRequestInput {
  ...
  deliveryDate?: string;
  priority?: 'fiyat' | 'hız' | 'kalite';
  notes?: string;
  language?: 'tr' | 'en';
}

export interface GeneratePurchaseRequestOutput {
  text: string;
  suggestions: string[];
  tokenUsage?: {
    totalTokens: number;
...
export async function generatePurchaseRequestText(
  input: GeneratePurchaseRequestInput,
  userId?: string,
  opts?: { provider?: AIProvider; model?: string; customInstructions?: string }
): Promise<GeneratePurchaseRequestOutput> {
  logger.group('AI: Generate Purchase Request Text');
  
  try {
    const language = input.language || 'tr';
    const provider: AIProvider = (opts?.provider as AIProvider) || (userId ? await getUserAIProvider(userId) : getDefaultProvider());
...
IMPORTANT: This is only an assistant suggestion, the final decision belongs to the user.`;
    ...
    const itemsText = input.items.map((item, idx) => {
      ...
      if (item.quantity) itemDesc += ` - Miktar: ${item.quantity}`;
      if (item.unit) itemDesc += ` ${item.unit}`;
      if (item.technicalSpec) itemDesc += ` - Teknik Şart: ${item.technicalSpec}`;
      return itemDesc;
    }).join('\n');
 ...
 */
export interface AnalyzeOfferComparisonInput {
  items: Array<{
    name: string;
    offers: Array<{
      supplierName: string;
  ...
  decisionCriteria?: Array<'fiyat' | 'hız' | 'kalite' | 'garanti' | 'ödeme'>;
  language?: 'tr' | 'en';
}

export interface AnalyzeOfferComparisonOutput {
  summary: string;
...
export async function analyzeOfferComparison(
  input: AnalyzeOfferComparisonInput,
  userId?: string,
  opts?: { provider?: AIProvider; model?: string; customInstructions?: string }
): Promise<AnalyzeOfferComparisonOutput> {
...
Analysis should be objective, corporate, and clear. IMPORTANT: This is only an assistant suggestion, the final decision belongs to the user.`;
    ...
    const itemsText = input.items.map((item, idx) => {
      const offersText = item.offers.map(offer => {
        ...
        if (offer.leadTimeDays) offerDesc += `, Teslim: ${offer.leadTimeDays} gün`;
        if (offer.warrantyMonths) offerDesc += `, Garanti: ${offer.warrantyMonths} ay`;
        if (offer.paymentTerms) offerDesc += `, Ödeme: ${offer.paymentTerms}`;
        if (offer.qualityNotes) offerDesc += `, Not: ${offer.qualityNotes}`;
        return offerDesc;
      }).join('\n');
...
Return in JSON format.`;
    ...
    // Parse JSON response
    let parsed: AnalyzeOfferComparisonOutput;
    try {
 ...
 */
export interface AutofillRequestFieldsInput {
  rawText?: string;
  previousRequests?: any[];
  knownMaterials?: Array<{
    code: string;
    name: string;
    defaultUnit?: string;
    defaultSpec?: string;
...
export interface AutofillRequestFieldsOutput {
...
export async function autofillRequestFields(
  input: AutofillRequestFieldsInput,
  userId?: string,
  opts?: { provider?: AIProvider; model?: string }
): Promise<AutofillRequestFieldsOutput> {
...
IMPORTANT: This is only an assistant suggestion, the final decision belongs to the user.`;
    ...
    const materialsText = input.knownMaterials?.map(m => 
      `${m.code}: ${m.name} (${m.defaultUnit || 'adet'})`
    ).join('\n') || '';
...
Return in JSON format.`;
    ...
    // Parse JSON response
    let parsed: AutofillRequestFieldsOutput;
    try {
      const jsonMatch = result.text.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        parsed = JSON.parse(jsonMatch[0]);
 ...
 */
export interface ValidateRequestForErrorsInput {
  items: Array<{
    name: string;
    quantity?: number;
    unit?: string;
...
export interface ValidateRequestForErrorsOutput {
  isValid: boolean;
  errors: string[];
  warnings: string[];
  suggestions: string[];
...
export async function validateRequestForErrors(
  input: ValidateRequestForErrorsInput,
  userId?: string,
  opts?: { provider?: AIProvider; model?: string }
): Promise<ValidateRequestForErrorsOutput> {
...
IMPORTANT: This is only an assistant suggestion, the final decision belongs to the user.`;
    ...
    const itemsText = input.items.map((item, idx) => {
      ...
      if (item.quantity) itemDesc += ` - Miktar: ${item.quantity}`;
      if (item.unit) itemDesc += ` ${item.unit}`;
      if (item.technicalSpec) itemDesc += ` - Şart: ${item.technicalSpec}`;
      return itemDesc;
    }).join('\n');
...
Return in JSON format.`;
    ...
    // Parse JSON response
    let parsed: ValidateRequestForErrorsOutput;
    try {
      const jsonMatch = result.text.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        parsed = JSON.parse(jsonMatch[0]);
