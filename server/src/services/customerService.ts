...
export function validateVKN(vkn: string): boolean {
  if (!/^\d{10,11}$/.test(vkn)) return false;
...
export interface CreateCustomerInput {
  ...
  email?: string; // Fatura için e-posta
  isEFaturaUser?: boolean; // Cache: e-Fatura mükellefi mi?
  efaturaQueryLastAt?: any; // Timestamp cache
  defaultScenario?: 'TEMEL' | 'TICARI'; // Varsayılan senaryo
  address?: {
    street?: string;
    avenue?: string;
    neighborhood?: string;
  ...
  creditLimit?: number;
  currency?: string;
  customerType?: 'cari' | 'one-time';
  isActive?: boolean;
  status?: 'pending' | 'approved' | 'rejected';
  notes?: string;
  supplierId?: string;
  userId: string; // createdBy için
}
...
export interface UpdateCustomerInput {
  ...
  efaturaQueryLastAt?: any;
  defaultScenario?: 'TEMEL' | 'TICARI';
  address?: {
    street?: string;
    avenue?: string;
    neighborhood?: string;
...
export async function updateCustomer(input: UpdateCustomerInput): Promise<void> {
  ...
  if (input.taxNumber !== undefined) updateData.taxNumber = input.taxNumber || null;
  if (input.taxOffice !== undefined) updateData.taxOffice = input.taxOffice || null;
  if (input.taxOfficeCode !== undefined) updateData.taxOfficeCode = input.taxOfficeCode || null;
  // Teklifbul Rule v1.0 - E-Belge alanları
  if (input.invoiceAddress !== undefined) updateData.invoiceAddress = input.invoiceAddress || null;
  if (input.email !== undefined) updateData.email = input.email || null;
  if (input.isEFaturaUser !== undefined) updateData.isEFaturaUser = input.isEFaturaUser || null;
  if (input.efaturaQueryLastAt !== undefined) updateData.efaturaQueryLastAt = input.efaturaQueryLastAt || null;
  if (input.defaultScenario !== undefined) updateData.defaultScenario = input.defaultScenario || null;
 ...
 * @returns Customer ID
 */
export async function findOrCreateCustomerFromSupplier(
  companyId: string,
  supplierId: string,
  userId: string
): Promise<string> {
