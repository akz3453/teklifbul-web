...
import { assertValidStatusTransition, isEditableStatus, SaleStatus } from '../services/saleStateMachine.js';
...
router.post('/',
  ...
  try {
    ...
    const targetStatus: SaleStatus = body.status || 'draft';
...
router.put('/:id',
  ...
  requireAnyPermission(['sales.edit', 'sales.edit_after_approve']),
  async (req: any, res) => {
    try {
      const userId = req.user?.uid;
      if (!userId) {
      ...
      const oldSale = saleDoc.data() as any;
      if (oldSale?.companyId !== body.companyId) {
        return res.status(403).json({ ok: false, error: 'Yetkisiz erişim' });
      }

      // Düzenleme kontrolü
      if (!isEditableStatus(oldSale.status as SaleStatus)) {
        return res.status(400).json({
          ok: false,
      ...
      let lockReason: 'sent_invoice' | 'sent_delivery' | 'sent_both' | null = null;
      ...
      if (invoiceIds.length > 0 || deliveryNoteIds.length > 0) {
        const invoiceDocs = await Promise.all(
          invoiceIds.map((id: string) => db.collection('invoices').doc(id).get())
        );
        const deliveryNoteDocs = await Promise.all(
          deliveryNoteIds.map((id: string) => db.collection('delivery_notes').doc(id).get())
        );

        const hasSentInvoice = invoiceDocs.some(
          (doc) => doc.exists && ['sent', 'accepted'].includes(doc.data()?.status)
        );
        const hasSentDeliveryNote = deliveryNoteDocs.some(
          (doc) => doc.exists && ['sent', 'accepted'].includes(doc.data()?.status)
        );
      ...
      if (body.status && body.status !== oldSale.status) {
        try {
          assertValidStatusTransition(oldSale.status as SaleStatus, body.status as SaleStatus);
      ...
      if (body.totalVat !== undefined) updateData.totalVat = body.totalVat;
      ...
      if (body.delivery !== undefined) updateData.delivery = body.delivery || null; // Teklifbul Rule v1.0 - Nakliye bilgileri
      ...
      try {
        await db.runTransaction(async (transaction) => {
          ...
          const currentSale = currentSaleDoc.data() as any;
 ...
 */
router.get('/:id', requirePermission('sales.view'), async (req: any, res) => {
  try {
    const userId = req.user?.uid;
    if (!userId) {
      return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.post('/:id/approve', requirePermission('sales.approve'), async (req: any, res) => {
  try {
    const userId = req.user?.uid;
    if (!userId) {
      return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.post('/:id/cancel', requirePermission('sales.cancel'), async (req: any, res) => {
  try {
    const userId = req.user?.uid;
    if (!userId) {
      return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.post('/:id/reject', requirePermission('sales.approve'), async (req: any, res) => {
  try {
    const userId = req.user?.uid;
    if (!userId) {
      return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 * Permission: sales.create
 */
router.post('/from-bid/:bidId', requirePermission('sales.create'), async (req: any, res) => {
  try {
    const userId = req.user?.uid;
    if (!userId) {
      return res.status(401).json({ ok: false, error: 'Unauthorized' });
 ...
 */
router.post('/:id/archive', requirePermission('sales.archive'), async (req: any, res) => {
  try {
    const userId = req.user?.uid;
    if (!userId) {
      return res.status(401).json({ ok: false, error: 'Unauthorized' });
...
router.delete('/:id', requirePermission('sales.edit'), async (req: any, res) => {
  try {
        ...
        try {
          ...
          if (invoiceDoc.exists) {
            ...
            if (invoice?.status === 'sent' || invoice?.status === 'accepted') {
              return respondError(
                res,
                Errors.conflict('E-belgeye bağlı satış silinemez; arşivleyin veya iptal/iade süreci kullanın.')
              );
            }
          }
        } catch (err) {
      ...
      for (const deliveryNoteId of deliveryNoteIds) {
        try {
          ...
          if (deliveryNoteDoc.exists) {
            const deliveryNote = deliveryNoteDoc.data() as any;
    ...
    return respondError(
      res,
      Errors.internal(error.message || 'Satış silinemedi')
    );
  }
});
 ...
 */
router.post('/:id/restore', requirePermission('sales.edit'), async (req: any, res) => {
  try {
    const userId = req.user?.uid;
    if (!userId) {
      return respondError(res, Errors.forbidden('Kimlik doğrulama gerekli'), 401);
        ...
        try {
          ...
          if (invoiceDoc.exists) {
            ...
            if (invoice?.status === 'sent' || invoice?.status === 'accepted') {
              return respondError(
                res,
                Errors.conflict('E-belgeye bağlı satış geri yüklenemez; iptal/iade süreci gereklidir.')
              );
            }
          }
        } catch (err) {
    ...
    return respondError(
      res,
      Errors.internal(error.message || 'Satış geri yüklenemedi')
    );
  }
});
...
router.post('/:saleId/invoice',
  ...
  saleDocumentCreationLimiter,
  requireAnyPermission(['sales.invoice', 'einvoice.create']),
  async (req: any, res) => {
    try {
      const userId = req.user?.uid;
      if (!userId) {
      ...
      const body = req.body as { companyId?: string; requestId?: string };
 ...
 */
router.post('/:saleId/delivery-note',
  validate({
    params: saleIdParamsSchema,
    body: createDeliveryFromSaleBodySchema
  }),
  saleDocumentCreationLimiter,
  requireAnyPermission(['sales.delivery', 'edespatch.create']),
  async (req: any, res) => {
    try {
      const userId = req.user?.uid;
      if (!userId) {
