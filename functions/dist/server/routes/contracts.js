...
router.get('/', validateRequest({ query: listQuerySchema }), async (req, res) => {
    ...
    try {
        ...
        if (missingSiteIds.length) {
            ...
            await Promise.all(missingSiteIds.map(async (sid) => {
                try {
            ...
            contracts = contracts.map((c) => c.siteId && !c.siteName && nameById.has(c.siteId)
        ...
        if (search) {
            ...
            contracts = contracts.filter((c) => (c.contractName || '').toLowerCase().includes(searchTerm) ||
 ...
 */
router.get('/:id', validateRequest({ params: contractIdParamsSchema }), async (req, res) => {
...
router.post('/', validateRequest({ body: createBodySchema }), async (req, res) => {
    ...
    try {
        ...
        const { contractNo, contractName, siteId, siteName, startDate, endDate, contractValue = 0, kdvRate = 20, stopajRate = 5, guaranteeRate = 6, advanceRate = 10, status = 'active', } = req.body;
        // Validasyon
        if (!contractName) {
 ...
 */
router.put('/:id', validateRequest({ params: contractIdParamsSchema, body: updateBodySchema }), async (req, res) => {
 ...
 */
router.delete('/:id', validateRequest({ params: contractIdParamsSchema }), async (req, res) => {
 ...
 */
router.get('/:id/items', validateRequest({ params: contractIdParamsSchema }), async (req, res) => {
 ...
 */
router.post('/:id/items', validateRequest({ params: contractIdParamsSchema, body: itemCreateBodySchema }), async (req, res) => {
    ...
    try {
        ...
        const { code, description, unit, unitPrice = 0, contractQtyLimit = null, } = req.body;
        // Validasyon
 ...
 */
router.put('/:id/items/:itemId', validateRequest({ params: itemIdParamsSchema, body: itemUpdateBodySchema }), async (req, res) => {
    ...
    try {
        ...
        const ITEM_UPDATE_KEYS = ['code', 'description', 'unit', 'unitPrice', 'contractQtyLimit'];
 ...
 */
router.delete('/:id/items/:itemId', validateRequest({ params: itemIdParamsSchema }), async (req, res) => {
 ...
 */
router.get('/:id/metraj-template', validateRequest({ params: contractIdParamsSchema }), async (req, res) => {
    ...
    try {
        ...
        res.setHeader('Content-Length', buffer.byteLength ?? buffer.length ?? 0);
