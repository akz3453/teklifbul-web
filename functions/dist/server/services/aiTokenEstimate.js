...
export function paidChatCompletionCap(provider, maxTokens) {
    if (typeof maxTokens === 'number' && Number.isFinite(maxTokens) && maxTokens > 0) {
        return Math.floor(maxTokens);
    }
    const key = String(provider || '').toLowerCase();
    if (key === 'gemini' || key.startsWith('gemini'))
        return PAID_CHAT_GEMINI_MAX_TOKENS;
...
export function estimatePaidChatTokens(params) {
    ...
    const charCount = String(params.systemPrompt || '').length + String(params.message || '').length;
