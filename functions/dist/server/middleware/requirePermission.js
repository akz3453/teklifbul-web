    ...
    return async (req, res, next) => {
        try {
            ...
            if (!userId) {
                ...
                return respondError(res, Errors.forbidden('Kimlik doğrulama gerekli', permKey), 401 // Override: 401 for auth required
                );
            }
            ...
            if (!companyId) {
                ...
                return respondError(res, Errors.forbidden('Sirket bilgisi bulunamadi', permKey));
            }
            ...
            if (!hasPerm) {
                ...
                return respondError(res, Errors.forbidden(`Bu işlem için yetkiniz yok. Gerekli yetki: ${permKey}. Rolünüz: ${userRole || 'belirlenemedi'}. Lütfen yöneticinizle iletişime geçin.`, permKey));
            }
            ...
            return respondError(res, Errors.internal('Yetki kontrolü sırasında hata oluştu'));
        }
    };
}
/**
    ...
    return async (req, res, next) => {
        try {
            ...
            if (!userId) {
                ...
                return respondError(res, Errors.forbidden('Kimlik doğrulama gerekli', permKeys.join(', ')), 401 // Override: 401 for auth required
                );
            }
            // CompanyId'yi al (async version with validation) — spoof → null, fail-closed
            const companyId = await getCompanyIdFromRequest(req);
            ...
            if (!hasPerm) {
                ...
                return respondError(res, Errors.forbidden('Bu işlem için yetkiniz yok', permKeys.join(', ')));
            }
            // Permission var, devam et
            next();
        }
