...
export interface TokenPackDefinition {
  ...
  price: number;            // Fiyat (TRY) - KDV dahil, Premium paket fiyatı hariç
  currency: string;         // Para birimi
  estimatedMessages?: number; // Tahmini mesaj sayısı (opsiyonel)
  description?: string;     // Paket açıklaması (opsiyonel)
  includesPremium?: boolean; // İlk satın almada Premium paket fiyatı dahil mi? (varsayılan: true)
}
 ...
 */
export const OPENAI_TOKEN_PACKS: TokenPackDefinition[] = [
  {
    id: 'openai_plus_1',
    name: 'PLUS-1',
    provider: 'openai',
 ...
 * Fiyatlar KDV dahil, Premium paket fiyatı hariç
 */
export const GEMINI_TOKEN_PACKS: TokenPackDefinition[] = [
  {
    id: 'gemini_mini',
    name: 'Mini Paket',
    provider: 'gemini',
 ...
 */
export function getAllTokenPacks(): TokenPackDefinition[] {
 ...
 */
export function getTokenPacksByProvider(provider: 'openai' | 'gemini'): TokenPackDefinition[] {
 ...
 */
export function getTokenPackById(id: string): TokenPackDefinition | null {
