...
router.get('/companies/subscriptions',
  validateRequest({
    query: z.object({
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const { planId = 'all', limit } = req.query as {
        planId?: string;
        limit?: number;
      };
      ...
      const companies = await Promise.all(companiesSnap.docs.map(async (doc: any) => {
        ...
        const normalizePlanValue = (val: any): string | null => {
...
router.get('/subscriptions',
  ...
  try {
    ...
    const { status = 'all', planId = 'all', limit } = req.query as {
      status?: string;
      planId?: string;
      limit?: number;
    };
    const items = await listSubscriptions({
      status: typeof status === 'string' ? (status as any) : 'all',
      planId: typeof planId === 'string' ? (planId as any) : 'all',
      limit: limit ? Number(limit) : undefined
    });
    
    // Kullanıcı bilgilerini ekle
    const db = await getAdminDb();
    const subscriptions = await Promise.all(items.map(async (item: any) => {
      if (db && item.userId) {
        try {
