...
export async function getAdminDb(): Promise<FirebaseFirestore.Firestore | null> {
  try {
      ...
      for (const candidate of serviceAccountPathHints) {
        try {
          if (existsSync(candidate)) {
            const serviceAccount = JSON.parse(readFileSync(candidate, 'utf8')) as ServiceAccount;
            credential = cert(serviceAccount);
    ...
    const projectId = (credential as any)?.projectId || process.env.FIREBASE_PROJECT_ID || 'teklifbul';
    initializeApp({
      credential,
      projectId
    });
 ...
 */
export async function getCollection(collectionName: string): Promise<FirebaseFirestore.CollectionReference | null> {
