...
import addrRouter from './routes/addr.js';
...
import aiTokenPurchasesRouter from './routes/ai-token-purchases.js';
import aiUsageReportRouter from './routes/ai-usage-report.js';
import billingPlanRouter from './routes/billing-plan.js';
import leadsRouter from './routes/leads.js';
import adminProfitRouter from './routes/admin-profit.js';
import adminProfitDetailRouter from './routes/admin-profit-detail.js';
import adminProfitAlertsRouter from './routes/admin-profit-alerts.js';
import adminGuardrailsRouter from './routes/admin-guardrails.js';
import adminAiHealthRouter from './routes/admin-ai-health.js';
import adminAiControlsRouter from './routes/admin-ai-controls.js'; // Teklifbul Rule v3.11 - Admin AI Controls
import adminEntitlementsRouter from './routes/admin-entitlements.js'; // Teklifbul Rule v3.14 - Admin Entitlements Debug
import adminFinanceSnapshotsRouter from './routes/admin-finance-snapshots.js'; // Teklifbul Rule v3.18 - Admin Finance Snapshots
import adminAutoProtectionRouter from './routes/admin-auto-protection.js'; // Teklifbul Rule v3.20 - Admin Auto-Protection
...
import paymentPreferenceRouter from './src/routes/payment-preference.js';
...
import suppliersMatchRouter from './routes/suppliers-match.js';
import userRouter from './routes/user.js';
import authRouter from './routes/auth.js';
import purchaseAssistantSettingsRouter from './routes/purchase-assistant-settings.js';
import purchaseAssistantRestorePaidRouter from './routes/purchase-assistant-restore-paid.js'; // Teklifbul Rule v3.5.2
...
import adminAiCatalogRouter from './routes/admin-ai-catalog.js';
import aiClassifyRouter from './routes/ai-classify.js';
import interimPaymentsRouter from './routes/interim-payments.js';
import contractsRouter from './routes/contracts.js';
import waybillRouter from './routes/waybills.js';
// Teklifbul Rule v1.0 - Tedarikçi teklif sistemi
import supplierQuotesRouter from './routes/supplier-quotes.js';
import internalRequestsRouter from './routes/internal-requests.js';
...
import tokenPacksRouter from './routes/token-packs.js';
...
app.post('/api/export/purchase-form', exportLimiter, verifyToken, async (req, res) => {
    try {
        ...
        // I2: Başlık
        if (demandData?.title)
            ws.getCell('I2').value = demandData.title;
        // K2: Talep Tarihi
        if (demandData?.demandDate)
            ws.getCell('K2').value = demandData.demandDate;
        // I4: Alım Yeri (İl)
        if (demandData?.purchaseLocation)
            ws.getCell('I4').value = demandData.purchaseLocation;
        // I1: Şantiye
        if (demandData?.siteName)
            ws.getCell('I1').value = demandData.siteName;
        // K1: Talep Tipi
        ...
        // K4: Öncelik
        if (demandData?.priority)
            ws.getCell('K4').value = demandData.priority;
        ...
        if (userRole === 'approver')
            ws.getCell('K11').value = userName || '';
        // H11: Genel Müdür
        if (demandData?.generalManager)
            ws.getCell('H11').value = demandData.generalManager;
        // G11: Satın Alma Yetkilisi
        if (userRole === 'satinalma_yetkilisi')
            ws.getCell('G11').value = userName || '';
        // F11: Talep Eden
        if (demandData?.requester)
            ws.getCell('F11').value = demandData.requester;
...
app.use(async (err, req, res, next) => {
    ...
    respondError(res, Errors.internal(process.env.NODE_ENV === 'development' ? (err?.message || String(err)) : 'Bir hata oluştu'));
});
...
app.use(async (req, res) => {
    ...
    const { Errors } = await import('./src/errors/errorCatalog.js');
    const { respondError } = await import('./src/errors/respondError.js');
    respondError(res, Errors.notFound(`Endpoint bulunamadı: ${req.method} ${req.path}`));
});
